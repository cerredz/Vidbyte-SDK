...
class VmaoTests(unittest.IsolatedAsyncioTestCase):
    async def test_executes_dag_and_verifies(self) -> None:
        ...
        worker = BaseAgent(name="worker", strategy=QueueStrategy("worker", ["answer-a", "answer-b"]))
        verifier = BaseAgent(
            name="verifier",
            strategy=QueueStrategy(
                "verifier",
    ...
    async def test_rejects_cycle(self) -> None:
        ...
        worker = BaseAgent(name="worker", strategy=QueueStrategy("worker", []))
        verifier = BaseAgent(name="verifier", strategy=QueueStrategy("verifier", []))
