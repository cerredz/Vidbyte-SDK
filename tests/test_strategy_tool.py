...
class StrategyToolSpecTests(unittest.TestCase):
    ...
    def test_spec_has_single_prompt_parameter(self) -> None:
        tool = self._tool()
        self.assertEqual(len(tool.spec().parameters), 1)
        self.assertEqual(tool.spec().parameters[0].name, "prompt")

    def test_prompt_parameter_is_required(self) -> None:
        tool = self._tool()
        self.assertTrue(tool.spec().parameters[0].required)
...
class StrategyToolExecuteTests(unittest.IsolatedAsyncioTestCase):
    async def test_execute_returns_strategy_output(self) -> None:
        agent = _make_agent(strategy=EchoStrategy())
        tool = StrategyTool(agent)
        result = await tool.execute(ToolCall("worker_strategy", {"prompt": "hello"}))
    ...
    async def test_execute_metadata_contains_strategy_name(self) -> None:
        agent = _make_agent(strategy=EchoStrategy())
        tool = StrategyTool(agent)
        result = await tool.execute(ToolCall("worker_strategy", {"prompt": "test"}))
    ...
    async def test_execute_returns_failure_on_strategy_error(self) -> None:
        ...
        tool = StrategyTool(agent)
        result = await tool.execute(ToolCall("worker_strategy", {"prompt": "anything"}))
    ...
    async def test_execute_returns_failure_when_no_real_runner(self) -> None:
        ...
        agent = BaseAgent(
            ...
            # No runner, no provider+model — results in ConfiguredAgentRunner with no-op config
        )
        # Force a ConfiguredAgentRunner by providing partial config
        agent.runner = ConfiguredAgentRunner(AgentRunnerConfig())
        tool = StrategyTool(agent)
        result = await tool.execute(ToolCall("no_runner_strategy", {"prompt": "test"}))
        self.assertEqual(result.status.value, "error")
