...
- Keep the continual trace agent under `vidbyte/trace/continual/` (`agent.py`, `tools.py`, `middleware.py`, and the `prebuilt/` package with one schema per module) and its config contracts in `vidbyte/lib/dataclasses/trace.py`; the `trace_option=` agent path is separate from the `trace=`/`tracer=` observability tracers, accepts one option or a sequence of options (one middleware per schema, results keyed under `metadata["traces"]`), and is documented in `skills/vidbyte-sdk/continual-tracing.md`. Follow that sub-skill when changing continual tracing behavior or adding prebuilt schemas.
