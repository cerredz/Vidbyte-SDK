...
| Permission models / sandbox contracts — `vidbyte/tools/security/`, `vidbyte/lib/dataclasses/security.py`, `sandbox.py` | P01-P05 | Proposed CodexPermissionTranslator and approval router | Preserve the stricter effective restriction and identify the enforcing process. A host callback may execute outside the native sandbox. |
