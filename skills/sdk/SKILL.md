...
- Keep the durable-sessions primitive self-contained under `vidbyte/sessions/`: the `Session` facade, `SessionStore` protocol + `BaseSessionStore`, `SessionScope`, serialization, trace capture, local stores (`vidbyte/sessions/stores/`), and the session contracts (`RunState`, `Checkpoint`, `SessionMeta`, enums, `SESSION_SCHEMA_VERSION` in `vidbyte/sessions/contracts.py`) and session errors (in `vidbyte/sessions/errors.py`). Expose the namespace via `sdk.harnesses.sessions`.
