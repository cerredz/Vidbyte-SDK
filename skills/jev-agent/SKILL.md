...
- `agent.py` maps settings into `BaseAgent`, fixes the runtime to `AgentRuntimeType.JEV`, and builds every run-time object a feature needs at construction: the `JevPreflightGate`, the `JevSpecialistRouter`, and the `JevResponse` writer. It passes them, with the settings the tool selector still reads, to the runtime through the single `_runtime_extension_kwargs()` hook, and exposes `JevAgent.response`, a `JevAgentResponse` record of what the features produced in the most recent run.
- `runtime.py` holds no fixed-question preset checks and no routing logic. `RuntimeRegistry` resolves `AgentRuntimeType.JEV` to `JevRuntime`, which refuses to build without a gate and a router. It calls `JevPreflightGate.pass_` first and returns `JevResponse.stopped()` when the gate closes; then it hands the run to `JevSpecialistRouter.run` when specialists are configured, with the general loop as a callable. The tool selector (`preflight.py`, `JevPreflightTools`) keeps its own path inside the general loop.
...
- `specialists.py` owns `JevSpecialistRouter`, the specialist-routing logic. The `JevSpecialist`, `JevSpecialistCatalog`, and `JevSpecialistRouting` records live in `vidbyte/lib/dataclasses/jev.py`, the fallback reasons (`JevSpecialistFallback`) in `vidbyte/lib/enums/jev.py`, and the limits in `vidbyte/lib/constants/jev.py`.
...
- `prompts.py` is the registry (`JevPrompt`, `JevPrompts`) for the text of the questions built at run time, specialist routing and the tool selector; the text itself lives in Markdown files under `vidbyte/prompts/jev/`, laid out as `skills/asking-jev-questions/SKILL.md` "Writing a full question" describes. Never inline question text in Python.
...
- `preflight.py` owns the internal `JevPreflight` contract and `JevPreflightTools` implementation.

With no preflight preset enabled and no specialist registered, a run performs no Jev call. A missing TypeSafe API key must not prevent `JevAgentSettings` or `JevAgent` construction. When an enabled preset cannot reach Jev, or a clarification cannot be written, the gate fails open: the preset is marked unavailable and the ordinary loop runs. `JevPreflightPreset.TOOL_SELECTOR` keeps every tool whose P(yes) is at least `tool_selector_threshold`, a finite probability from 0 through 1 inclusive. An empty specialist catalog makes no routing call; with specialists configured, a missing key or failed decision falls back to the general model loop, and `JevAgent.response.routing` says why.
...
5. Implement preset orchestration in `JevPreflightGate` and the steps under `vidbyte/agents/jev/gate/`, and any other capability's logic in its own class under `vidbyte/agents/jev/` built in `JevAgent.__init__`; never add preset checks or capability policy to `JevRuntime`, which only calls each class in order. Keep provider wire shapes in `vidbyte/providers/typesafe.py` and reusable validated records in `vidbyte/lib/`.
...
- Runtime state is run-local; reusable configuration is frozen and validated before execution, and every preflight and routing object is built in `JevAgent.__init__`.
...
- Specialist routing runs only after the gate passes. Specialist templates are forked per routed run. A weak match or decision failure falls back to general; a specialist execution failure does not replay the task.
