...
- Add a public `vidbyte/trace/` package with a tracer client facade and split tracer implementation modules.
...
1. Import `Trace` from `vidbyte.trace.base`.
2. Import `DebugTracer` from `vidbyte.trace.debug`.
3. Import `ContinualTracer` from `vidbyte.trace.continual`.
4. Expose a small `__all__` with only public facade types.
...
Defines the public `Trace` tracer client facade. Concrete tracer logic lives in `vidbyte/trace/debug.py`, `vidbyte/trace/continual/`, and existing provider adapters.
...
**File(s):** `vidbyte/trace/debug.py`
**Type:** New file

#### What it does

Implements `DebugTracer`, the in-memory `TracerBase` used for local inspection and tests.
...
1. `DebugTracer` owns or receives a list of event dictionaries.
2. `DebugTracer.start_trace(...)` records `{"type": "start_trace", "name": name, "attributes": dict(attributes), "context": context}` and returns a `SpanContext`.
3. `DebugTracer.end_trace(...)` records output/error metadata without raising.
4. `DebugTracer.start_span(...)` records span start with parent metadata and returns a `SpanContext`.
5. `DebugTracer.end_span(...)` records span end output/error metadata without raising.

---

### 6.4 Continual Trace Package

**File(s):** `vidbyte/trace/continual/__init__.py`, `vidbyte/trace/continual/base.py`
**Type:** New files

#### What it does

Implements `ContinualTracer` in a dedicated continual tracing package reserved for future trace memory behavior.
...
1. `ContinualTracer.__init__` validates `remember` and `max_memory_chars`.
2. It stores `remember` as a tuple, stores `redact`, and delegates event recording behavior to `DebugTracer`.
3. The package exposes `ContinualTracer` from `vidbyte.trace.continual`.

---

### 6.5 Trace Factory Helper
...
| CREATE | `vidbyte/trace/base.py` | Public `Trace` tracer client and custom tracer validation |
| CREATE | `vidbyte/trace/debug.py` | `DebugTracer` in-memory tracing implementation |
| CREATE | `vidbyte/trace/continual/__init__.py` | Continual tracing package exports |
| CREATE | `vidbyte/trace/continual/base.py` | `ContinualTracer` settings and validation |
