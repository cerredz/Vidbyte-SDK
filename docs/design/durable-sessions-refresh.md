...
2. **Consolidate session logic in `vidbyte/sessions/`.** Move the session-owned dataclasses (`RunState`, `Checkpoint`, `SessionMeta`, enums, `SESSION_SCHEMA_VERSION`) and the session error family out of `vidbyte/lib/` into `vidbyte/sessions/`, so the entire resume/checkpoint/fork surface — contracts, errors, scope, store, serializer, trace recorder, facade, local stores — lives in one package. Non-session provider adapters (Mongo/Supabase/Postgres session stores) stay in `vidbyte/lib/providers/` per the existing lazy-driver convention.
...
2. `RunState`, `Checkpoint`, `SessionMeta`, `CheckpointPolicy`, `SessionStatus`, `TraceCapture`, and `SESSION_SCHEMA_VERSION` are importable from `vidbyte.sessions` (and `vidbyte.sessions.contracts`); they are **no longer** importable from `vidbyte.lib.dataclasses`.
...
vidbyte/sessions/
    ...
    contracts.py         # NEW: RunState, Checkpoint, SessionMeta, enums, SESSION_SCHEMA_VERSION  (moved from lib/dataclasses/sessions.py)
...
- `lib/dataclasses/__init__.py`: keep main's `sources` re-exports; **remove** the `vidbyte.lib.dataclasses.sessions` import block and its `__all__` entries (contracts moved to `vidbyte/sessions/contracts.py`).
...
**File:** `vidbyte/sessions/contracts.py` (new), `vidbyte/lib/dataclasses/sessions.py` (deleted)
**Type:** New + Delete

Move `RunState`, `Checkpoint`, `SessionMeta`, `CheckpointPolicy`, `SessionStatus`, `TraceCapture`, `SESSION_SCHEMA_VERSION` verbatim into `vidbyte/sessions/contracts.py` with an updated Context Protocol Header. Delete `vidbyte/lib/dataclasses/sessions.py`. Field shapes, the frozen+slots dataclass declarations, and the persisted JSON shape from the original design doc §7.1 are unchanged.
...
`BaseAgent`:
- `export_state()`/`restore()` already present; update the `RunState` import to `vidbyte.sessions.contracts`.
...
- `skills/sdk/SKILL.md`: rewrite the durable-sessions rule to:
  - "Keep all durable-session logic (facade, `SessionStore` protocol + `BaseSessionStore`, `SessionScope`, serialization, trace capture, local stores, session dataclasses `RunState`/`Checkpoint`/`SessionMeta`/enums, and session errors) under `vidbyte/sessions/`. Expose the namespace via `sdk.harnesses.sessions`."
...
Update imports from `vidbyte.lib.dataclasses.sessions` / `vidbyte.lib.errors` to `vidbyte.sessions.contracts` / `vidbyte.sessions.errors`. Remove test cases that exercise the removed `SessionTool` (replaced by builtins). Do **not** add new test cases for the builtins (no-tests workflow); note this as a follow-up. `scripts/test-durable-sessions.py` is unchanged (it loads `tests.test_durable_sessions` by name).
...
- **Moved (same names, new import paths):** `RunState`, `Checkpoint`, `SessionMeta`, `CheckpointPolicy`, `SessionStatus`, `TraceCapture`, `SESSION_SCHEMA_VERSION` now at `vidbyte.sessions.contracts` (re-exported from `vidbyte.sessions`); `SessionError` family now at `vidbyte.sessions.errors` (re-exported from `vidbyte.sessions`).
...
| DELETE | `vidbyte/lib/dataclasses/sessions.py` | Contracts moved to `vidbyte/sessions/contracts.py` |
| CREATE | `vidbyte/sessions/contracts.py` | RunState/Checkpoint/SessionMeta/enums (moved) |
...
- What: leave `RunState`/`Checkpoint`/`SessionMeta` in `lib/dataclasses/sessions.py` and `SessionError` in `lib/errors/`, only confirming the logic is in `vidbyte/sessions/`.
