...
### Implementation update from PR review

The implemented boundary is stricter than the original draft: the resolved chain is represented by `AgentFallbackConfig` in `vidbyte/lib/dataclasses`, policy kinds come from `FallbackPolicyType`, and runtime switches are represented by `FallbackTransitionRequest`/`FallbackTransition`. `AgentRuntime` delegates to `AgentFallback.fallback_transition` rather than owning validation or attempt mutation. These contracts raise `FallbackConfigurationError` and `FallbackTransitionError` from `vidbyte/lib/errors`.
...
- Put fallback model identity, policy kinds, policy dimensions, and runtime transition inputs in shared `vidbyte/lib` dataclasses, enums, and errors so direct chain callers and the runtime use the same strict contract.
...
- **Moving `FallbackModel`/`FallbackTransform` into the new package.** These stay in `vidbyte/lib/dataclasses/agents.py`, alongside `AgentRunnerConfig`, `AgentRuntimeConfig`, `AgentCard`, and other agent-wide data contracts. `lib/dataclasses/` is the SDK's shared, dependency-free contract layer that `vidbyte/agents/*` (a higher layer) imports from; moving two of many peer dataclasses out to sit next to their consumer would invert that layering for no functional benefit — confirmed as more than theoretical during implementation, when a similar layering inversion (co-locating `AgentFallbackSettings` inside `agents/fallback/`) produced a real circular import that had to be fixed by deferring `Runner`'s import (Section 6.2).
...
Review follow-up: `AgentFallbackConfig` now owns the resolved-chain model/error/policy contract, while `FallbackTransitionRequest` owns strict in-flight state validation. `AgentRuntime` only assembles that request and applies the returned `FallbackTransition`; it does not duplicate validation or audit-record mutation. The shared enum and custom errors live under `vidbyte/lib`, preserving the dependency direction from agent packages into the contract layer.
...
AgentFallbackSettings(models=[...], policies=[LatencyPolicy([...]), CostBudgetPolicy([...])])
        |  to_fallback(primary=...)
        v
AgentFallback(AgentFallbackConfig(models=(primary, fb1, fb2, ...), fallback_on=..., policies=(...)))
        |
...
Review boundary: both triggers enter `AgentFallback.fallback_transition(FallbackTransitionRequest)`. That shared method validates the in-flight request, records one credential-free attempt, and calls the existing transform logic. The runtime only applies the returned `FallbackTransition`.
...
from vidbyte.lib.dataclasses.agents import AgentFallbackConfig, FallbackTransform

__all__ = ["AgentFallback", "AgentFallbackConfig", "AgentFallbackSettings", "CostBudgetPolicy", "DEFAULT_FALLBACK_ERRORS", "FallbackTransform", "LatencyPolicy"]
```
...
#### Logic / Algorithm
1. `__init__` accepts only `AgentFallbackConfig`; that shared frozen dataclass normalizes collections and validates models, error classes, policy kinds, policy dimensions, and positive finite values before the chain stores them.
...
#### Logic / Algorithm
1. `_validate()` retains early public-input checks and delegates shared policy validation to `AgentFallbackConfig.validate_policies` using the declared fallback count.
2. `to_fallback()` resolves the primary plus declared entries, constructs `AgentFallbackConfig(models=resolved, fallback_on=..., policies=...)`, and passes that contract to `AgentFallback`.
3. The dataclass contract repeats the same policy rules for direct internal callers, including the resolved-chain arithmetic and custom `FallbackConfigurationError` type.
...
Their arrays are validated only after attachment to a chain. `AgentFallbackConfig` owns that shared validation; settings invokes the same method early for developer-facing construction errors.
...
#### Logic / Algorithm
`_fallback_transition` and `_cost_fallback_transition` now build `FallbackTransitionRequest` and delegate to `AgentFallback.fallback_transition`. The runtime applies the returned transform and publishes the audit span; validation, attempt mutation, and exhaustion remain in the fallback chain.
...
New SDK-facing contract exports include `AgentFallbackConfig`, `FallbackTransitionRequest`, `FallbackTransition`, `FallbackPolicyType`, `FallbackConfigurationError`, and `FallbackTransitionError`; the fallback package continues to expose its existing chain/settings/policy names.
...
| CREATE | `vidbyte/lib/enums/fallback.py` | Closed policy-kind enum used by shared validation |
| MODIFY | `vidbyte/lib/dataclasses/agents.py` | Shared fallback config, transition request, and result contracts |
| MODIFY | `vidbyte/lib/errors/base.py` | Custom fallback configuration and transition errors |
