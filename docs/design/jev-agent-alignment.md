...
2. **Assess (Jev).** `JevAgentAlignment.run(JevAlignmentInput(...))` asks the questions in `alignment/questions.py`. The input keeps the user's task in `user_prompt`, the agent instructions in `system_prompt`, and actual Vidbyte tool objects in `tools`. Each registry entry is a `JevAlignmentQuestion` dataclass with a `noul` question and explicit true/false criteria. The positive phrasing means `true` always means aligned.
   ...
   - *Dynamic* questions (does the request fit the scope, are its terms and output covered) use state `{system_prompt, user_prompt, tools}`.
...
5. **Edit.** If there are agent gaps, the editor runs its ordinary loop with one tool from `vidbyte/tools/edit_system_prompt_section.py`. A context variable binds the tool to this call's `JevPromptDraft`; the draft stores section edits in the SDK `ContextManager` registry, replacing a prior edit for the same section. Edits are additive only: content goes under a `## <Section>` heading, which is created if missing. Owner text is never replaced or deleted. The tool rejects locked sections, fixes that don't cite an open gap for that section, blank content, and content over the size caps. The editor's instructions are a prompt asset (`jev_alignment.editor_system_prompt`).
...
7. **Run.** `JevRuntime.arun` swaps the original prompt prefix in `context.system_prompt` for the edited prompt. It also sets the run-local runtime's `system_prompt`, so context-window hooks see the same prompt. The returned `JevResponse` carries feature data in `response.alignment`, exposes the prompt that ran as `response.aligned_prompt`, and provides `aligned_prompt` as a convenience property; the shared records live in `vidbyte/lib/dataclasses/jev_alignment.py`.
...
Callers read the run result without unpacking metadata:
...
- New `vidbyte/agents/jev/alignment/`: `__init__.py`, `questions.py`, `draft.py`, `result.py`, `agent.py`.
- New shared records in `vidbyte/lib/dataclasses/jev_alignment.py` and the SDK edit tool in `vidbyte/tools/edit_system_prompt_section.py`.
- New prompt family `vidbyte/prompts/prompts/jev_alignment/` and a `Prompt.JEV_ALIGNMENT_EDITOR_SYSTEM_PROMPT` enum member.
- Modify `vidbyte/agents/jev/settings.py` (`self_align`), `agent.py` (builds the alignment), `runtime.py` (align, then run), `BaseAgent` (preserve the typed message subclass), and the `jev`/`agents`/root exports.
