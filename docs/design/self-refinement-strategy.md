...
- No task-specific few-shot prompt library in this PR; reusable SDK prompt templates are limited to the Self-Refinement strategy prompts.
...
15. Self-Refinement prompt templates must be exposed as classes under `vidbyte.prompts.strategies` with `export()` methods.
16. README and SDK skill docs must document the strategy API.
17. The Obsidian note `Vidbyte/product/prompt_engineering_strategies.md` must gain a Self-Refine section.
...
The strategy will live under `vidbyte/strategies/agent_loops/self_refinement.py` and implement the existing `BaseStrategy` contract from PR #2. It will be a prompt/API-only strategy: every create, feedback, and refine step calls `TextModelRunner.run()` with a task-specific prompt class from `vidbyte.prompts.strategies` and optional system prompt.
...
| CREATE | `vidbyte/prompts/__init__.py` | Public prompt package exports |
| CREATE | `vidbyte/prompts/strategies/__init__.py` | Strategy prompt exports |
| CREATE | `vidbyte/prompts/strategies/self_refinement.py` | Reusable Self-Refinement prompt classes |
| MODIFY | `vidbyte/strategies/base.py` | Add `BaseStrategyUtils` for shared helper methods |
...
Summary: 6 files created, 6 files modified, 0 files deleted.
...
- `SelfRefinementStrategyTests.test_exposes_prompt_classes_for_reuse`
