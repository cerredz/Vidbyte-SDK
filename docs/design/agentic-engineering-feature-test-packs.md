...
Add a numbered principle entry:

```markdown
5. Feature Test Packs as Executable Intent
   [Summary paragraph]
...
```markdown
system_prompt, error_messages, file_headers, folder_readme, function_design, feature_test_packs
```
...
- [ ] If `aggressive_linting` or `intent_based_commenting` PRs are opened or merged before implementation, should this PR rebase and renumber the system prompt entry? Recommendation: yes, renumber on the implementation branch against current `main`; numbering is display-only.
