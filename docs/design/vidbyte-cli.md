# Design Doc: Minimal `vidbyte` CLI (skills subcommand)
...
Add a unified `vidbyte` console command to the SDK, implemented as a small stdlib-argparse package at `vidbyte/cli/`, wired through a single `[project.scripts]` entry point in `pyproject.toml`. Its first and only subcommand group is `skills` (`vidbyte skills list|show|install`), a thin wrapper over the `Skills` registry designed in `docs/pre-design/paradigm-skills-and-registry.md`. The CLI is deliberately minimal (~100ΓÇô150 lines, zero new dependencies) but structured with a subcommand-registration seam so future groups ΓÇö e.g. commands that talk to the main Vidbyte backend ΓÇö slot in without restructuring.

**Dependency note for the implementer:** this design depends on the skills registry design doc. Read `docs/pre-design/paradigm-skills-and-registry.md` (it is promoted to `docs/design/` once implemented) and look for these files in the repo: `vidbyte/skills/catalog.py` (`Skills`, `SkillRecord`), `vidbyte/lib/enums/skills.py` (`Skill` enum), and `vidbyte/paradigms/context_minimal_fanout/skills/` (assets + `skills.json`). If they do not exist yet, implement that doc first ΓÇö this CLI has nothing to wrap without it.

---

## 2. Original User Prompts

Prompts 1ΓÇô3 established the paradigm skills + registry feature and are recorded verbatim in `docs/pre-design/paradigm-skills-and-registry.md` ┬º2. Copied here again for self-containment:
...
> I think that the cleanest way we can do it is this:   2. One line in pyproject ΓÇö a real command. Add vidbyte-skills =
...
1. **Unified `vidbyte` command, not `vidbyte-skills`.** The user chose the unified entry point (`vidbyte = "vidbyte.cli:main"`) with `skills` as the first subcommand group, explicitly because the SDK may later hook up to the main Vidbyte backend and more subcommands are expected. One brand command that grows beats a constellation of `vidbyte-*` binaries.
2. **CLI lives at `vidbyte/cli/` (inside the package), not repo-top-level `cli/`.** This is the software-engineering-best-practice answer the user asked for, for three hard reasons: (a) `[project.scripts]` entry points must reference an *importable module path* ΓÇö `vidbyte.cli:main` only resolves if the code is inside the `vidbyte` package; (b) `pyproject.toml` packaging uses `[tool.setuptools.packages.find] include = ["vidbyte*"]`, so a top-level `cli/` directory would silently not ship in the wheel; (c) even if force-included, installing a top-level package literally named `cli` into site-packages would squat a generic name and collide with other distributions ΓÇö a known packaging anti-pattern. Precedent in-repo: the existing console script `vidbyte-mcp-server = "vidbyte.mcp_server.__main__:main"` already follows the inside-the-package pattern.
...
4. **Minimal command surface for v1**: `vidbyte skills list`, `vidbyte skills show <key>`, `vidbyte skills install <key> --dest <dir>`. The user said "very minimalistic CLI for this one command" ΓÇö the load-bearing command is `install`; `list` is required for discoverability; `show` is cheap and lets users read a skill before installing. Nothing else.
5. **Subcommand-registration seam for growth.** Each subcommand group is its own module (`vidbyte/cli/skills.py`) exposing a `register(subparsers)` function; `vidbyte/cli/__init__.py`'s `main()` builds the root parser and calls each group's `register`. Adding a future `backend`/`auth`/`prompts` group = one new module + one register call. No plugin machinery, no dynamic discovery ΓÇö an explicit list is fine at this scale.
6. **The CLI is a thin adapter over the registry ΓÇö zero business logic.** All listing/reading/materializing behavior lives in `Skills` (per the registry design doc). The CLI translates argv ΓåÆ catalog calls ΓåÆ stdout/exit codes. If the CLI needs a capability the catalog lacks, the capability goes in the catalog.
...
- **`vidbyte-skills` as a separate console script** ΓÇö rejected by the user in favor of the unified command (Prompt 4 quotes both options and picks unified).
- **Repo-top-level `cli/` directory** ΓÇö rejected; see Key Decision 2 (not importable as `vidbyte.cli`, not shipped by `packages.find`, generic-name squatting).
- **click / typer** ΓÇö rejected; unnecessary dependency for this surface.
- **A separate `vidbyte-sdk-skills` PyPI package** ΓÇö rejected earlier in the conversation: nothing to escape (SDK deps are only pydantic + httpx), and a second package means version-sync drift. `pip install <name>` only installs files; the command comes from the entry point either way.
- **`python -m vidbyte.skills` as the primary interface** (option 1 from the conversation) ΓÇö superseded by the real entry point; keeping both argparse surfaces would duplicate code.
- **Harness-aware install (`--harness claude --scope user` auto-resolving paths like `~/.claude/skills/`)** ΓÇö deferred to keep v1 minimal; `--dest` is explicit and universal. Recorded as future work (see Open Questions), since the harness path table already exists conceptually in the prompts README / PR #154 work.

### Constraints & Assumptions

- **Assumes the skills registry exists** per `docs/pre-design/paradigm-skills-and-registry.md`: `Skills` with `keys()`, `descriptions()`, `get()`, `text()`, `files()`, `materialize(key, dest_dir)`; `Skill` str-Enum; `ConfigurationError` on invalid assets. If the implemented API drifted from that doc, wrap what actually exists ΓÇö do not re-implement.
...
- Q (user): `vidbyte-sdk/cli` or `vidbyte-sdk/vidbyte/cli` ΓÇö which is best practice? A: `vidbyte-sdk/vidbyte/cli` (inside the package), for the three reasons in Key Decision 2. This doc treats that as settled.
- Q (user, from Prompt 2): do we need "an entire CLI"? A: no framework needed ΓÇö one entry-point line in pyproject plus an argparse `main()`. This doc is that answer made concrete.
- Assumption made without asking: `show` prints the SKILL.md body to stdout; `list` prints one `key ΓÇö description` line per skill. If the user wants different output shapes, it's a trivial change at implementation review time.
...
- **Materialize / install**: `Skills.materialize(key, dest_dir)` ΓÇö write the skill folder (SKILL.md + any shared references) under `dest_dir`. The CLI verb is `install`; the catalog verb is `materialize`.
...
- **Entry-point precedent**: `pyproject.toml` `[project.scripts]` already has `vidbyte-mcp-server = "vidbyte.mcp_server.__main__:main"`. Add `vidbyte = "vidbyte.cli:main"` beside it. Look at `vidbyte/mcp_server/__main__.py` for the house style of a `main()` module.
- **Suggested file layout** (3 files + README):
  - `vidbyte/cli/__init__.py` ΓÇö `main(argv: list[str] | None = None) -> int`: builds root parser (`prog="vidbyte"`, `--version` from package metadata via `importlib.metadata.version("vidbyte-sdk")`), attaches subparsers, calls each group's `register`, dispatches to the selected handler, returns exit code.
  - `vidbyte/cli/skills.py` ΓÇö `register(subparsers)` adding `skills` with actions `list`, `show <key>`, `install <key> --dest <dir> [--force]`; handlers instantiate `Skills()` lazily (inside handlers, not at import) so `vidbyte --help` stays fast and never trips catalog validation.
  - `vidbyte/cli/__main__.py` ΓÇö `raise SystemExit(main())` so `python -m vidbyte.cli` works.
  - `vidbyte/cli/README.md` ΓÇö folder README in house format.
- **`main()` must accept an argv parameter** and return an int rather than calling `sys.exit` internally (except in `__main__.py`) ΓÇö this is what makes the CLI unit-testable without subprocesses.
...
- **Do NOT touch**: `vidbyte/mcp_server/`, the paradigm packages, the prompts catalog, or the `Skills` catalog itself (unless a missing capability forces a catalog addition ΓÇö in which case add it there, with tests, not in the CLI).

### Open Questions

1. **Harness-aware install** (`--harness claude|codex|opencode|antigravity --scope user|project` resolving destination paths automatically) ΓÇö future work; the path table exists in the prompts README / PR #154 context. v1 ships `--dest` only.
...
3. `--json` output flag on `list`/`show` for scripting ΓÇö cheap, but omitted from v1 requirements; add only if the implementer finds it trivial.
4. Command name collision check: `vidbyte` as a console script is assumed free on the user's PATH (it is the brand name). No known conflict; if PyPI/PATH conflicts surface, fall back to `vidbyte-cli` ΓÇö user decision at that point.
...
- A `vidbyte` console command installed with `pip install vidbyte-sdk`, via one `[project.scripts]` line.
- `vidbyte skills list` ΓÇö enumerate all registry skills with descriptions.
- `vidbyte skills show <key>` ΓÇö print a skill's SKILL.md to stdout.
- `vidbyte skills install <key> --dest <dir>` ΓÇö materialize a skill folder to disk via `Skills.materialize`.
- `python -m vidbyte.cli` parity; `vidbyte --version`; testable `main(argv) -> int`.
...
- No `vidbyte/skills/__main__.py` (explicitly superseded ΓÇö see Key Decision 7).
...
- **Current state:** the repo has one console script (`vidbyte-mcp-server`) proving the entry-point path works end-to-end. There is no `vidbyte/cli/` package and (as of this doc) the skills registry itself is not yet implemented ΓÇö implementation order is registry first, CLI second.

---

## 6. Requirements

1. `pyproject.toml` gains `vidbyte = "vidbyte.cli:main"` under `[project.scripts]`; the existing `vidbyte-mcp-server` entry is unchanged.
...
4. `vidbyte --version` prints the installed `vidbyte-sdk` version (via `importlib.metadata`) and exits 0.
5. `vidbyte skills list` prints one line per registry skill ΓÇö key plus description ΓÇö sourced from `Skills`; exits 0.
6. `vidbyte skills show <key>` prints the skill's SKILL.md text to stdout; exits 0.
7. `vidbyte skills install <key> --dest <dir>` calls `Skills.materialize`, creating `<dir>` if needed, and prints the written skill path; exits 0. With an existing non-empty target it exits 1 with an error unless `--force` is given.
...
9. Expected failures (`ConfigurationError`, filesystem errors, unknown keys) produce a one-line stderr message and a nonzero exit ΓÇö never a traceback.
10. `Skills` is instantiated only inside command handlers, so `vidbyte --help` and `vidbyte skills --help` succeed even if catalog assets are broken.
11. `python -m vidbyte.cli <args>` behaves identically to `vidbyte <args>`.
12. `tests/test_cli_interface.py` covers requirements 3ΓÇô11 by invoking `main()` in-process (no subprocesses), using `tmp_path` for installs and `capsys` for output.
...
- **Performance:** `vidbyte --help` must not import or validate the skills catalog (lazy handler-level instantiation); target sub-200ms help output on a warm interpreter.
- **Scalability:** adding a subcommand group must require only a new module with `register(subparsers)` plus one call in `main()` ΓÇö no refactor.
...
- **Observability:** N/A ΓÇö stdout/stderr and exit codes are the whole interface; keep stderr messages one-line and prefixed `error:`.
...
```
 ...
 vidbyte/skills/catalog.py Skills.keys()/descriptions()/text()/materialize()
        ...
        v
 vidbyte/paradigms/context_minimal_fanout/skills/ΓÇª  packaged assets
        |
        v
