...
2. `AgentSpeedTracker.record_tool_call(RecordToolCallInput) -> ToolCallSpeedRecord | None` records one tool call's timing and whether it timed out, returning `None` on an internal validation failure rather than raising — this method is called from `_execute_tool`'s `finally` block, where a raised exception would replace whatever original exception was already propagating.
3. `AgentSpeedTracker.record_step(RecordStepInput) -> StepSpeedRecord | None` records one loop iteration's timing, same fail-open contract; implemented and unit-tested, not yet called from `runtime.py` (Non-Goal).
...
4. `record_tool_call(call_input)` re-validates on construction inside a `try`/`except AgentSpeedValidationError` even though `RecordToolCallInput` already validated its fields — defense in depth, because this method is invoked from a `finally` block where a second exception would mask the original one. Appends and returns a `ToolCallSpeedRecord`, or marks corrupted and returns `None`.
5. `record_step(step_input)` follows the same fail-open pattern for consistency, even though nothing calls it yet (Non-Goal).
...
- [x] `fallback_index`'s exact source variable/initialization in `runtime.py`'s outer loop was verified during implementation: it is a plain `int` local, `fallback_index = 0` initialized before the `while True:` loop, reassigned to `transition.index` on a fallback transition. §6.7's snippet is wired exactly as designed.
...
### Phase 5.5 refinement outcomes (resolved during implementation)

- **[Notable, fixed]** `record_tool_call`/`record_step` originally shared `record_call`'s dataclass-input contract but not its fail-open behavior: a validation failure inside the constructed record would have raised `AgentSpeedValidationError` straight out of `_execute_tool`'s `finally` block, masking whatever original exception was already propagating. Both methods now wrap construction in the same `try`/`except AgentSpeedValidationError` pattern as `record_call`, returning `None` and marking the tracker corrupted instead. A regression test (`test_record_tool_call_never_raises_when_the_clock_goes_backwards`) proves this with a deliberately regressing fake clock.
- **[Critical, fixed]** The initial implementation introduced six new SDK agent-lint regressions (A001 structured headers, A002 intent comments, A003 error diagnostic fields, A007 named constants, C002 duplicated bool guard, S009 mypy arg-type). All six are fixed; verified via a throwaway `origin/main` worktree comparison that this branch's full `python lint/run.py` output is byte-for-byte identical to a pristine checkout — every remaining "REGRESSED" line is pre-existing drift between `lint/baseline.json` and `origin/main` HEAD, unrelated to this change.
- **[Minor, disclosed, not changed]** `AgentSpeedTracker.now()` returns a bare `float` rather than a dataclass, since it takes no input and its output has no internal structure to validate — see §6.5 point 1 for the full justification (mirrors `MiddlewarePipeline.clock()`'s identical bare-float precedent).
- **[Minor, considered, not changed]** `_parallelism_efficiency`/`_framework_overhead_ms` use plain `sum()`/`max()`/`min()` builtins rather than routing through `MathHelper`, because `MathHelper` was scoped to statistical aggregation (mean, percentile, max-of-a-list, argmax) per the explicit instruction, not arithmetic totals or boundary-timestamp lookups; wrapping trivial one-line builtin calls would be ceremony with no shared-formula benefit.
