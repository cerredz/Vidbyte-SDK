...
- Add `preflight` to `JevAgentSettings`; keep it disabled by default and reject duplicate or unknown presets during settings validation.
...
- Increase the SDK's local per-request Jev question cap from 16 to 64 so the clarity preset and future small preset combinations can remain a single call. This is a local safety bound, not a claimed TypeSafe limit.

## Files

- Create `vidbyte/agents/jev/presets.py` and `vidbyte/agents/jev/response.py`.
- Modify Jev settings, runtime, package exports, root exports, and the local Jev question cap.
