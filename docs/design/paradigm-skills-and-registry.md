...
5. **Skill document structure**: YAML frontmatter (`name`, `description` — required by Claude Code skill discovery) followed by Goal, Description, Use Cases, Algorithm (numbered step-by-step), and Rules sections. This satisfies the user's requested sections ("goal, description, use cases, and step by step/instructions/algorithm") while staying consistent with the action-oriented shape of `vidbyte/prompts/prompts/templates/master.md` and the existing SKILL.md house style (e.g. `skills/context-minimal-fanout/SKILL.md` uses identity/intent/workflow/rules XML sections; the new skills should keep the flat Goal/Use-Cases/Algorithm/Rules headings the user asked for, since these are distributable end-user skills, not contributor skills).
...
- Ship four distributable skill folders under `vidbyte/paradigms/context_minimal_fanout/skills/`, each with frontmatter + Goal / Description / Use Cases / Algorithm / Rules sections.
...
2. Every `SKILL.md` has YAML frontmatter with non-empty `name` (matching its folder name) and `description`, followed by sections: Goal, Description, Use Cases, Algorithm (numbered steps), Rules.
...
5. A `Skill` str-Enum exists in `vidbyte/lib/enums/skills.py` with one member per skill (values namespaced by paradigm, e.g. `context_minimal_fanout.decompose_fanout`).
...
```
 ...
 vidbyte/lib/enums/skills.py (Skill enum)
        |
        +--> dev code: Skills().text(Skill.CMF_DECOMPOSE_FANOUT)
