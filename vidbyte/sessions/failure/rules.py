...
ARCHITECTURE NOTE: Decoration is metadata-only; registration is explicit on one Session failure router.
    Hooks reuse vidbyte.lib.dataclasses.middleware.MiddlewareHook, the same closed vocabulary the
    runtime's own AgentMiddleware lifecycle already uses, instead of a second local set of hook strings.
...
from vidbyte.lib.dataclasses.middleware import MiddlewareHook
...
class FailureRule:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "on", MiddlewareHook(self.on))
...
def rule(*, code: FailureCode, on: MiddlewareHook, on_match: FailureDisposition = FailureDisposition.ROUTE, on_error: RuleErrorMode = RuleErrorMode.OPEN, priority: int = 0, name: str = "") -> Callable[[_RuleFunction], _RuleFunction]:
