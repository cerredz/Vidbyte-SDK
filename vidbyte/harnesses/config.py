...
ARCHITECTURE NOTE:
    ...
    (schema.py); per-agent deep validation of provider, model, and parameters is
    delegated to the dataclass AgentSettings (vidbyte.lib.dataclasses.config) which
    validates against the SDK's canonical ProviderModelRegistry and typed field
    contracts. This file keeps only what is not declarative validation: JSON
...
WHAT NOT TO DO IN THIS FILE:
    ...
    6. Do not re-add per-agent provider/model/param checks; AgentSettings owns them.
...
class HarnessConfigLoader:
    ...
    def _validate_structure(self, normalized: Mapping[str, Any]) -> dict[str, Any]:
        ...
        config["agents"] = self._validate_agent_entries(config["agents"])
        return config
    ...
    def _validate_agent_entries(self, agents: list[dict[str, Any]]) -> list[dict[str, Any]]:
        validated: list[dict[str, Any]] = []
        for index, item in enumerate(agents):
            validated.append(self._validate_agent(item, index))
        return validated
    ...
    def _validate_agent(self, item: Any, index: int) -> dict[str, Any]:
        ...
        self._validate_agent_via_settings(agent, index)
        return agent

    def _validate_agent_via_settings(self, agent: dict[str, Any], index: int) -> None:
        """Validates provider, model, and params against the canonical AgentSettings dataclass."""
        prompt = agent.get("system_prompt")
        payload: dict[str, Any] = {"name": agent["name"]}
        ...
        if "model" in agent:
            payload["model_name"] = agent["model"]
        if "params" in agent:
            for key, val in agent["params"].items():
                payload[str(key)] = val
        if "tools" in agent:
            payload["tools"] = agent["tools"]
        try:
            AgentSettings.from_mapping(payload, f"agents[{index}]")
        except ConfigurationError as exc:
            raise HarnessConfigurationError(
                f"Harness agent at agents[{index}] failed validation: {exc}",
                details={
                    "field": f"agents[{index}]",
                    "agent_name": agent.get("name"),
                    "violation": exc.details if exc.details else str(exc),
                },
            ) from exc
