...
from vidbyte.context import VMAOContext
from vidbyte.lib.dataclasses.multi_agent import DagNode, NodeState, Verification
...
class VerifiedMultiAgentOrchestrationStrategy(BaseMultiAgentStrategy):
    ...
    def __init__(self, *, planner: BaseAgent, workers: Sequence[BaseAgent], verifier: BaseAgent, synthesizer: BaseAgent | None = None, max_rounds: int = 3, completeness_threshold: float = 0.85, max_parallel_tasks: int = 4) -> None:
    ...
    async def arun(self, prompt: str, *, runner: object | None = None, context: StrategyContext | None = None, tools: Sequence[object] = (), **options: Any) -> StrategyResult:
        ...
        for round_index in range(1, self.max_rounds + 1):
            vmao_context = self._build_vmao_context(context, round_index)
            nodes = await self._plan(current_prompt, context=vmao_context)
            round_states = await self._execute_dag(nodes, context=context, options=options)
            all_states.extend(round_states)
            final_output = await self._synthesize(prompt, all_states, context=vmao_context)
            verification = await self._verify(prompt, final_output, context=vmao_context)
    ...
    async def _plan(self, prompt: str, *, context: StrategyContext | None) -> tuple[DagNode, ...]:
        planner_prompt = VMAOPrompts("planner").export(prompt=prompt, context=context.build_context() if context else "")
        reply = await self._run_agent(self.planner, planner_prompt, context=context, recipient="vmao")
        try:
            return self._parse_nodes(reply.content)
        except StrategyExecutionError:
            repair_prompt = VMAOPrompts("planner_repair").export(raw=reply.content)
            repair = await self._run_agent(self.planner, repair_prompt, context=context, recipient="vmao")
            return self._parse_nodes(repair.content)

    async def _execute_dag(self, nodes: Sequence[DagNode], *, context: StrategyContext | None, options: dict[str, Any]) -> list[NodeState]:
    ...
    async def _execute_node(self, state: NodeState, states: dict[str, NodeState], *, context: StrategyContext | None, options: dict[str, Any]) -> str:
    ...
    async def _synthesize(self, original_prompt: str, states: Sequence[NodeState], *, context: StrategyContext | None) -> str:
        evidence = "\n\n".join(f"{state.node.id}: {state.output}" for state in states)
        if self.synthesizer is None:
            return f"Original task:\n{original_prompt}\n\nCollected results:\n{evidence}"
        prompt = VMAOPrompts("synthesizer").export(prompt=original_prompt, evidence=evidence)
    ...
    async def _verify(self, original_prompt: str, output: str, *, context: StrategyContext | None) -> Verification:
        prompt = VMAOPrompts("verifier").export(prompt=original_prompt, output=output, context=context.build_context() if context else "")
    ...
    def _gap_prompt(self, original_prompt: str, output: str, verification: Verification) -> str:
        ...
        return VMAOPrompts("gap_planner").export(prompt=original_prompt, output=output, gaps=gaps)

    def _build_vmao_context(self, context: StrategyContext | None, round_index: int) -> VMAOContext:
        return VMAOContext(
            ...
            strategy_metadata={**dict(context.strategy_metadata if context else {}), "vmao_round": round_index},
        ...
        for item in parsed:
            ...
            nodes.append(
                DagNode(
                    ...
                    preferred_capability=str(item["preferred_capability"]) if item.get("preferred_capability") else None,
                )
            )
        return tuple(nodes)
