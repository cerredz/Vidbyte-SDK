...
# Intent
The intent of file headers is to make every source file explain its own architectural contract before an agent spends context reading implementation details. A file header is the first navigational surface an agent sees, so it should answer whether this is the right file to edit, what this file owns, what it depends on, what depends on it, and what mistakes must not be made here.

This principle is trying to make source files agent-native by turning the top of each file into a durable orientation layer. Instead of forcing agents to reconstruct purpose from imports, names, and partial code reads, the header states the file's role, boundaries, modification paths, and edge cases explicitly. The result is faster routing, fewer wrong-file edits, and less context wasted on rediscovering facts that should be embedded in the file itself.
