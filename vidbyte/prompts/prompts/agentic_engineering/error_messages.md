...
# Intent
The intent of agent-native error messages is to give the agent enough context to solve the error by itself, without needing to open additional files, search logs, or ask a human for clarification. When a runtime failure occurs, the first thing an agent does is read the error object — that is the primary bootstrap artifact for every debugging cycle. If the error carries only a short message string, the agent must search broadly for the root cause, often loading dozens of files before it understands what failed and why. A well-formed error packet reverses this: it states the failure mode precisely, identifies the invariant that was violated, captures the runtime state that triggered the failure, names the files most likely to contain the root cause, ranks the hypotheses by probability, and lists the repair strategies that have worked before. The goal is to compress an entire debugging session — what would otherwise take an agent many reads and inferences — into a single structured read of the exception object. Each field in the error serves this goal: the description replaces a broad code search, the expected_vs_actual field localizes the invariant, the blast_radius tells the agent what else might be broken, and the fix_approaches give the agent a starting point rather than a blank slate.

This principle is designed to make backend failures self-diagnosing rather than opaque dead ends that stop agent progress. Agents are fundamentally weakest when they must reconstruct context from a vague message, a partial stack trace, and a codebase they may be reading for the first time. Every second the agent spends searching for what the error means is a second it is not spending on the fix. Rich error packets eliminate this search cost by encoding diagnostic information at the point of definition, where the author knows the failure mode best and can describe it precisely. The custom error class pattern — one class per failure mode with static fields for all invariant context — means the diagnostic information travels with the exception and never goes stale with respect to the throw site. An agent that catches a typed, information-rich exception can identify the problem domain, understand what state was wrong, and select a repair strategy without loading a single additional file. The result is a category of engineering investment that pays compound interest: every hour spent writing a complete error class eliminates hours of future agent debugging across every future invocation of that failure mode.
