...
class RewindTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Move the current (bound) session's head back to an earlier checkpoint so the next "
                "run branches from that point (time-travel). Only valid for checkpoints in this "
                "session. Returns the new head checkpoint id."
            ),
            ...
            input_schema={
                ...
                "properties": {
                    "checkpoint_id": {"type": "string", "description": "The earlier checkpoint id in the bound session to rewind to."},
                },
            },
        )
