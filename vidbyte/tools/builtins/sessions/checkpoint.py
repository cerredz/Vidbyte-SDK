...
class CheckpointTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Write a checkpoint of a durable session. With no session_id, snapshots the current "
                "(bound) agent thread. With an in-scope session_id, copies that session's head state "
                "into a new labeled checkpoint on that session. Returns the new checkpoint id."
            ),
            ...
            input_schema={
                ...
                "properties": {
                    "session_id": {"type": "string", "description": "Target session id for a cross-thread checkpoint. Omit to checkpoint the current thread."},
