...
class ResumeAppendTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Resume another agent's thread by APPENDING its context window into the current one. "
                "The other thread's history is added as a framed block after the current history, so "
                "the current context is preserved. Requires an in-scope session_id. Returns the new "
                "head checkpoint id."
            ),
            ...
            input_schema={
                ...
                "required": ["session_id"],
                "properties": {
                    "session_id": {"type": "string", "description": "Target session id whose context window to append."},
                    "checkpoint_id": {"type": "string", "description": "Specific checkpoint to append from. Defaults to the target session's head."},
                },
            },
        )
