...
class ResumeOutputTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Resume another agent's thread by appending ONLY its final output to the current "
                "context. The target session must be completed (COMPLETED status); an unfinished "
                "thread returns an error. Requires an in-scope session_id. Returns the new head "
                "checkpoint id."
            ),
            ...
            input_schema={
                ...
                "required": ["session_id"],
                "properties": {
                    "session_id": {"type": "string", "description": "Target completed session id whose final output to append."},
                },
            },
        )
