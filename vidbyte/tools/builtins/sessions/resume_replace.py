...
class ResumeReplaceTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Resume a thread by REPLACING the current context window. With no session_id, rewinds "
                "the current (bound) thread to an earlier checkpoint_id (own-thread time-travel). With "
                "an in-scope session_id, overrides the current context with that session's checkpointed "
                "history. Returns the new head checkpoint id."
            ),
            ...
            input_schema={
                ...
                "properties": {
                    "session_id": {"type": "string", "description": "Target session id whose thread to adopt. Omit to rewind the current thread."},
                    "checkpoint_id": {"type": "string", "description": "Specific checkpoint to resume from. Defaults to the target session's head or, for own-thread, is required."},
                },
            },
        )
