...
class ForkTool(_SessionBuiltinTool):
    ...
    def spec(self) -> ToolSpec:
        ...
        return ToolSpec(
            name=_TOOL_NAME,
            description=(
                "Branch a new durable session from a checkpoint. With no arguments, forks the current "
                "(bound) thread from its head. With an in-scope checkpoint_id, forks that checkpoint "
                "into a new session id with parent lineage recorded. Returns the new session id."
            ),
            ...
            input_schema={
                ...
                "properties": {
                    "session_id": {"type": "string", "description": "Target session id for a cross-thread fork. Omit to fork the current thread."},
                    "checkpoint_id": {"type": "string", "description": "Specific checkpoint to fork from. Defaults to the bound session's head or the target session's head."},
                },
            },
        )
