...
FILE: vidbyte/tools/builtins/reasoning/fishbone_trace.py
PURPOSE: Defines the fishbone-trace model-facing reasoning trace tool and its immutable strategy definition.
...
Description:
    Defines the fishbone-trace strategy-specific reasoning trace tool.
Purpose:
...
Architecture:
    - FishboneTraceTool: Strategy-owned description and parameter shape.
...
class FishboneTraceTool(ReasoningTraceTool):
    ...
    definition = ReasoningTraceDefinition(
        ...
        parameter(name='verification', type='string', description="Verification captures the verification dimension of the strategy for the fishbone trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for categorize and investigate possible root causes. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='confidence', type='number', description="Confidence captures the calibrated confidence in the current working direction for the fishbone trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for categorize and investigate possible root causes. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='next_action', type='string', description="Next action captures the next observable action that will advance or test the work for the fishbone trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for categorize and investigate possible root causes. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        ),
    )


__all__ = ['FishboneTraceTool']
