...
FILE: vidbyte/tools/builtins/reasoning/influence_diagram_trace.py
PURPOSE: Defines the influence-diagram-trace model-facing reasoning trace tool and its immutable strategy definition.
...
Description:
    Defines the influence-diagram-trace strategy-specific reasoning trace tool.
Purpose:
...
Architecture:
    - InfluenceDiagramTraceTool: Strategy-owned description and parameter shape.
...
class InfluenceDiagramTraceTool(ReasoningTraceTool):
    """Model-facing influence-diagram-trace reasoning trace tool."""

    definition = ReasoningTraceDefinition(
        skill_name='influence-diagram-trace',
        purpose='Map decisions, uncertainties, objectives, and dependencies.',
        description='Use the influence diagram reasoning trace when the current task benefits from map decisions, uncertainties, objectives, and dependencies. Its central move gives the public checkpoint a strategy-specific structure for examining the question. The parameters separate the observations, judgments, uncertainties, and actions that make the strategy inspectable. The tool writes one bounded context primitive so later iterations can recover the declared reasoning state. Use it at a meaningful checkpoint after the relevant inputs are available and before the next action is chosen. The record does not execute the strategy, verify model-authored claims, or replace authoritative task instructions. Treat the result as auditable telemetry that supports comparison across iterations without exposing private chain-of-thought.',
        parameters=(
        parameter(name='question', type='string', description="Question captures the focused question or decision target for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='decisions', type='string', description="Decisions captures the decisions dimension of the strategy for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='uncertainties', type='string', description="Uncertainties captures the uncertainties dimension of the strategy for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='objectives', type='string', description="Objectives captures the objectives dimension of the strategy for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='dependencies', type='array', description="Dependencies captures the prerequisites and relationships that constrain execution for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='value_of_information', type='string', description="Value of information captures the value of information dimension of the strategy for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='recommendation', type='string', description="Recommendation captures the recommendation dimension of the strategy for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='confidence', type='number', description="Confidence captures the calibrated confidence in the current working direction for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='next_action', type='string', description="Next action captures the next observable action that will advance or test the work for the influence diagram trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for map decisions, uncertainties, objectives, and dependencies. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        ),
    )


__all__ = ['InfluenceDiagramTraceTool']
