...
FILE: vidbyte/tools/builtins/reasoning/deductive_trace.py
PURPOSE: Defines the deductive-trace model-facing reasoning trace tool and its immutable strategy definition.
...
Description:
    Defines the deductive-trace strategy-specific reasoning trace tool.
Purpose:
...
Architecture:
    - DeductiveTraceTool: Strategy-owned description and parameter shape.
...
class DeductiveTraceTool(ReasoningTraceTool):
    ...
    definition = ReasoningTraceDefinition(
        ...
        parameter(name='confidence', type='number', description="Confidence captures the calibrated confidence in the current working direction for the deductive trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for derive necessary conclusions from stated premises. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='next_action', type='string', description="Next action captures the next observable action that will advance or test the work for the deductive trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for derive necessary conclusions from stated premises. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        ),
    )


__all__ = ['DeductiveTraceTool']
