...
FILE: vidbyte/tools/builtins/reasoning/constraint_removal_trace.py
PURPOSE: Defines the constraint-removal-trace model-facing reasoning trace tool and its immutable strategy definition.
...
Description:
    Defines the constraint-removal-trace strategy-specific reasoning trace tool.
Purpose:
...
Architecture:
    - ConstraintRemovalTraceTool: Strategy-owned description and parameter shape.
...
class ConstraintRemovalTraceTool(ReasoningTraceTool):
    """Model-facing constraint-removal-trace reasoning trace tool."""

    definition = ReasoningTraceDefinition(
        skill_name='constraint-removal-trace',
        purpose='Temporarily remove assumed constraints to discover options.',
        description='Use the constraint removal reasoning trace when the current task benefits from temporarily remove assumed constraints to discover options. Its central move gives the public checkpoint a strategy-specific structure for examining the question. The parameters separate the observations, judgments, uncertainties, and actions that make the strategy inspectable. The tool writes one bounded context primitive so later iterations can recover the declared reasoning state. Use it at a meaningful checkpoint after the relevant inputs are available and before the next action is chosen. The record does not execute the strategy, verify model-authored claims, or replace authoritative task instructions. Treat the result as auditable telemetry that supports comparison across iterations without exposing private chain-of-thought.',
        parameters=(
        parameter(name='question', type='string', description="Question captures the focused question or decision target for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='current_state', type='string', description="Current state captures the present condition being measured or improved for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='target_state', type='string', description="Target state captures the condition the process is intended to reach for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='dependencies', type='array', description="Dependencies captures the prerequisites and relationships that constrain execution for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='constraint', type='string', description="Constraint captures the condition that limits feasible choices or performance for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='bottleneck', type='string', description="Bottleneck captures the limiting factor that governs overall throughput for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='controls', type='string', description="Controls captures the comparison or safeguard used to isolate the relevant effect for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='confidence', type='number', description="Confidence captures the calibrated confidence in the current working direction for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        parameter(name='next_action', type='string', description="Next action captures the next observable action that will advance or test the work for the constraint removal trace. It keeps the record anchored to the strategy's central move rather than a generic account of reasoning. It separates this dimension from the other observations and judgments needed for temporarily remove assumed constraints to discover options. Record the material content that would change how a careful reviewer interprets the current state. Keep the value explicit enough that a later iteration can compare it with new evidence and revise it. Treat this field as model-authored telemetry whose usefulness depends on honest, bounded reporting."),
        ),
    )


__all__ = ['ConstraintRemovalTraceTool']
