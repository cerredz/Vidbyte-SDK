...
Relations:
    Related to vidbyte.tools.base, vidbyte.tools.registry, and vidbyte.tools.executor.
"""
...
class ToolSpec:
    ...
    def __post_init__(self) -> None:
        """Validate the tool name and normalize permission enum inputs."""
        if not self.name.strip():
    ...
    def to_prompt_str(self) -> str:
        """Render compact tool documentation for insertion into prompts."""
        lines = [f"Tool: {self.name}", f"Description: {self.description}"]
        if self.parameters:
        ...
        lines.append(f"Permission: {self.permission.value}")
        return "\n".join(lines)
