...
class Skills:
    ...
    def get(self, key: Skill) -> SkillRecord:
        ...
        if not isinstance(key, tuple(SkillEnums.values())):
            raise TypeError("Skills.get() expects a registered skill enum member.")
    ...
    def _validate_enum_sync(records: Mapping[Skill, SkillRecord]) -> None:
        ...
        missing_assets = sorted(skill.value for skill in iter_skill_values() if skill not in records)
        if missing_assets:
