...
Key methods: both __post_init__ methods validate eagerly; prompt methods render the
    role-specific system contract and exact-evidence user request.
...
class SpecialistRole:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "specialist_id", _required_text(self.specialist_id, "specialist_id"))
        object.__setattr__(self, "responsibility", _required_text(self.responsibility, "responsibility"))
        object.__setattr__(self, "instructions", _required_text(self.instructions, "instructions"))
        object.__setattr__(self, "output_requirements", _unique_text(self.output_requirements, "output_requirements", require_one=True))
        object.__setattr__(self, "tool_names", _unique_text(self.tool_names, "tool_names"))
        object.__setattr__(self, "artifact_names", _unique_text(self.artifact_names, "artifact_names"))
        _validate_provider_model(self.provider, self.model)
        object.__setattr__(self, "reviewer_options", _frozen_mapping(self.reviewer_options, "reviewer_options", reject_reserved=True))
...
class SpecialistPanelAlgorithm:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_template(self.reviewer_system_prompt, "reviewer_system_prompt", _SYSTEM_PLACEHOLDERS)
        _validate_template(self.reviewer_prompt, "reviewer_prompt", _USER_PLACEHOLDERS)
        object.__setattr__(self, "metadata", _frozen_mapping(self.metadata, "metadata"))
    ...
    def _validate_roles(self) -> None:
        ...
        if not 2 <= len(self.roles) <= _MAX_ROLES:
            raise ConfigurationError(f"roles must contain between 2 and {_MAX_ROLES} specialists; found {len(self.roles)}.")
        if any(not isinstance(role, SpecialistRole) for role in self.roles):
            raise ConfigurationError("roles must contain only SpecialistRole values.")
        ids = tuple(role.specialist_id.strip().casefold() for role in self.roles)
        responsibilities = tuple(_normalize_comparison(role.responsibility) for role in self.roles)
        if len(set(ids)) != len(ids):
            raise ConfigurationError("specialist_id values must be unique after trimming and case folding.")
        if len(set(responsibilities)) != len(responsibilities):
            raise ConfigurationError("responsibilities must be unique after whitespace normalization and case folding.")
    ...
    def _validate_limits(self) -> None:
        ...
        if not 1 <= threshold <= len(self.roles):
            raise ConfigurationError(f"min_successful must be between 1 and {len(self.roles)}; found {threshold}.")
        if not 0 < self.reviewer_timeout_seconds <= _MAX_TIMEOUT_SECONDS:
            raise ConfigurationError(f"reviewer_timeout_seconds must be greater than zero and at most {_MAX_TIMEOUT_SECONDS}.")
        if not 1 <= self.reviewer_max_iterations <= _MAX_REVIEWER_ITERATIONS:
            raise ConfigurationError(f"reviewer_max_iterations must be between 1 and {_MAX_REVIEWER_ITERATIONS}.")
        for name in ("reviewer_max_tokens", "reviewer_max_tool_calls"):
            value = getattr(self, name)
            if value is not None and value <= 0:
                raise ConfigurationError(f"{name} must be greater than zero when provided.")
        for name in ("max_task_chars", "max_candidate_chars", "max_artifact_chars", "max_review_chars"):
            value = getattr(self, name)
            if not 0 < value <= _MAX_TEXT_LIMIT:
                raise ConfigurationError(f"{name} must be greater than zero and at most {_MAX_TEXT_LIMIT}.")
        if not 1 <= self.max_findings_per_role <= 256:
            raise ConfigurationError("max_findings_per_role must be between 1 and 256.")
...
def _required_text(value: str, field_name: str) -> str:
    ...
    if len(cleaned) > limit:
        raise ConfigurationError(f"{field_name} values must contain at most {limit} characters; found {len(cleaned)}.")
    return cleaned
...
def _unique_text(values: Sequence[str], field_name: str, *, require_one: bool = False) -> tuple[str, ...]:
    ...
    if require_one and not normalized:
        raise ConfigurationError(f"{field_name} must contain at least one non-empty value.")
    if len(normalized) > _MAX_ROLE_COLLECTION_ITEMS:
        raise ConfigurationError(f"{field_name} must contain at most {_MAX_ROLE_COLLECTION_ITEMS} unique values; found {len(normalized)}.")
...
def _validate_provider_model(provider: str | None, model: str | None) -> None:
    ...
    if (provider is None) != (model is None):
        raise ConfigurationError("provider and model must be provided together.")
...
def _frozen_mapping(value: Mapping[str, Any], field_name: str, *, reject_reserved: bool = False) -> Mapping[str, Any]:
    ...
    for key, item in dict(value).items():
        if not isinstance(key, str):
            raise ConfigurationError(f"{field_name} keys must be strings; found {type(key).__name__}.")
        if reject_reserved and key.casefold() in _RESERVED_REVIEWER_OPTIONS:
            raise ConfigurationError(f"{field_name} contains reserved context-shaping key {key!r}.")
...
def _freeze_json_value(value: Any, field_name: str) -> Any:
    ...
    raise ConfigurationError(f"{field_name} must contain only JSON-like values; found {type(value).__name__}.")
...
def _validate_template(template: str | None, field_name: str, required: frozenset[str]) -> None:
    ...
    if not template.strip():
        raise ConfigurationError(f"{field_name} must be non-empty when provided.")
    if len(template) > _MAX_PROMPT_TEMPLATE_CHARS:
        raise ConfigurationError(f"{field_name} must contain at most {_MAX_PROMPT_TEMPLATE_CHARS} characters; found {len(template)}.")
    found = {name for _, name, _, _ in string.Formatter().parse(template) if name}
    missing = sorted(required - found)
    if missing:
        raise ConfigurationError(f"{field_name} is missing required formatting placeholders: {missing}.")
