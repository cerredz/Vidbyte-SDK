...
FUNCTION INVENTORY: CritiqueAdjudicateReviseAlgorithm validates configuration, parses
    ...
    owns exact per-stage allowlists and execution limits. No new tests are added by the
    approved no-tests workflow; existing regressions and manual probes cover this API.
...
_MAX_STAGE_INPUT_CHARS = 2_000_000
_MAX_STAGE_ITERATIONS = 100
_MAX_STAGE_TOOL_CALLS = 100
_FINDING_CATEGORIES = frozenset({"correctness", "requirements", "security", "performance", "evidence", "clarity", "other"})
_FINDING_SEVERITIES = frozenset({"critical", "high", "medium", "low"})
_EVIDENCE_SOURCE_KINDS = frozenset({"task", "candidate", "artifact", "tool"})
_REJECTION_REASONS = frozenset({"unsupported", "duplicate", "contradicted", "out_of_scope", "not_actionable"})
_RESERVED_SOURCES = frozenset({"original_task", "candidate"})
...
class ReviewStageAccess:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "allowed_artifact_names", _normalize_names(self.allowed_artifact_names, "allowed_artifact_names"))
        object.__setattr__(self, "allowed_tool_names", _normalize_names(self.allowed_tool_names, "allowed_tool_names"))
        _require_bounded_positive(self.max_iterations, "max_iterations", _MAX_STAGE_ITERATIONS)
        _require_bounded_positive(self.max_tool_calls, "max_tool_calls", _MAX_STAGE_TOOL_CALLS)
...
class CritiqueAdjudicateReviseAlgorithm:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "critic_failure_policy", _coerce_enum(self.critic_failure_policy, CriticFailurePolicy, "critic_failure_policy"))
        object.__setattr__(self, "adjudication_failure_policy", _coerce_enum(self.adjudication_failure_policy, StageFailurePolicy, "adjudication_failure_policy"))
        object.__setattr__(self, "revision_failure_policy", _coerce_enum(self.revision_failure_policy, StageFailurePolicy, "revision_failure_policy"))
    ...
    def parse_critic_output(self, critic_id: str, payload: str, sources: Mapping[str, str], tool_calls: Sequence[ToolCallContext]) -> tuple[CriticFinding, ...]:
        ...
        return _StructuredReviewParser(self).parse_critic(critic_id, payload, sources, tool_calls)
    ...
    def validate_adjudication(self, payload: str, findings: Sequence[CriticFinding]) -> _AdjudicationProjection:
        ...
        return _StructuredReviewParser(self).parse_adjudication(payload, findings)
    ...
    def parse_revision_output(self, payload: str, accepted: Sequence[AcceptedFinding]) -> tuple[str, tuple[str, ...]]:
        ...
        return _StructuredReviewParser(self).parse_revision(payload, accepted)

    def _validate_counts(self) -> None:
        # Rejects unbounded critic fan-out and internally inconsistent quorum policy.
        if isinstance(self.critic_count, bool) or not isinstance(self.critic_count, int) or not 1 <= self.critic_count <= _MAX_CRITICS:
            raise ConfigurationError(f"critic_count must be between 1 and {_MAX_CRITICS}.")
    ...
    def _validate_limits(self) -> None:
        ...
        _require_bounded_positive(self.max_findings_per_critic, "max_findings_per_critic", _MAX_FINDINGS)
        _require_bounded_positive(self.max_evidence_per_finding, "max_evidence_per_finding", _MAX_EVIDENCE)
        _require_bounded_positive(self.max_field_chars, "max_field_chars", _MAX_FIELD_CHARS)
        _require_bounded_positive(self.max_candidate_chars, "max_candidate_chars", _MAX_CANDIDATE_CHARS)
        _require_bounded_positive(self.max_stage_input_chars, "max_stage_input_chars", _MAX_STAGE_INPUT_CHARS)
...
class _StructuredReviewParser:
    ...
    def _parse_finding(self, critic_id: str, finding_index: int, raw: object, sources: Mapping[str, str], tool_calls: Sequence[ToolCallContext]) -> CriticFinding:
        ...
        category = self._enum_text(obj["category"], "critic.finding.category", _FINDING_CATEGORIES)
        severity = self._enum_text(obj["severity"], "critic.finding.severity", _FINDING_SEVERITIES)
    ...
    def _parse_evidence(self, finding_id: str, evidence_index: int, raw: object, sources: Mapping[str, str], tool_calls: Sequence[ToolCallContext]) -> FindingEvidence:
        ...
        source_kind = self._enum_text(obj["source_kind"], "critic.evidence.source_kind", _EVIDENCE_SOURCE_KINDS)
    ...
    def _ground_evidence(self, source_kind: str, source_name: str, locator: str, excerpt: str, sources: Mapping[str, str], tool_calls: Sequence[ToolCallContext]) -> None:
        ...
        if source_kind == "artifact":
            if source_name in _RESERVED_SOURCES or source_name not in sources:
    ...
    def _parse_rejected_group(self, raw: object, finding_map: Mapping[str, CriticFinding]) -> tuple[tuple[str, ...], str]:
        ...
        reason = self._enum_text(obj["reason_code"], "adjudicator.rejected_group.reason_code", _REJECTION_REASONS)
