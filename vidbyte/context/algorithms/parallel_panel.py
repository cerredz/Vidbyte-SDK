...
class ParallelPanelAlgorithm:
    ...
    def __post_init__(self) -> None:
        ...
        self._validate_integer_range("reviewer_count", self.reviewer_count, 2, MAX_REVIEWERS)
        self._validate_integer_range("min_successful_reviews", self.min_successful_reviews, 2, self.reviewer_count)
        if self.max_concurrency is not None:
            self._validate_integer_range("max_concurrency", self.max_concurrency, 2, self.reviewer_count)
        self._validate_optional_timeout("per_reviewer_timeout_seconds", self.per_reviewer_timeout_seconds)
        self._validate_optional_timeout("panel_timeout_seconds", self.panel_timeout_seconds)
        self._validate_character_limit("max_candidate_chars", self.max_candidate_chars, MAX_CANDIDATE_CHARS)
        self._validate_character_limit("max_review_chars", self.max_review_chars, MAX_REVIEW_CHARS)
        self._validate_character_limit("max_artifact_chars", self.max_artifact_chars, MAX_ARTIFACT_CHARS)
        self._validate_character_limit("max_total_artifact_chars", self.max_total_artifact_chars, MAX_TOTAL_ARTIFACT_CHARS)
        self._validate_artifact_names(self.artifact_names)
        self._validate_optional_prompt("reviewer_system_prompt", self.reviewer_system_prompt)
        self._validate_optional_prompt("reviewer_prompt", self.reviewer_prompt)
        self._validate_review_prompt_fields(self.reviewer_prompt)
    ...
    def _validate_integer_range(field_name: str, value: int, minimum: int, maximum: int) -> None:
        ...
        if value < minimum or value > maximum:
            raise ConfigurationError(
                ...
                details={"field": field_name, "minimum": minimum, "maximum": maximum, "actual": value},
            )
    ...
    def _validate_optional_timeout(field_name: str, value: float | None) -> None:
        ...
        if not math.isfinite(float(value)) or float(value) <= 0 or float(value) > MAX_TIMEOUT_SECONDS:
            raise ConfigurationError(
                f"{field_name} must be greater than zero and no more than {MAX_TIMEOUT_SECONDS:g} seconds.",
                details={"field": field_name, "actual": value},
            )

    @classmethod
    def _validate_character_limit(cls, field_name: str, value: int, maximum: int) -> None:
        # Rejects character limits that cannot bound content safely and predictably.
        cls._validate_integer_range(field_name, value, 1, maximum)
    ...
    def _validate_artifact_names(names: tuple[str, ...]) -> None:
        ...
        for index, name in enumerate(names):
            ...
            if len(name) > MAX_ARTIFACT_NAME_CHARS:
                raise ConfigurationError(
                    f"artifact_names entries cannot exceed {MAX_ARTIFACT_NAME_CHARS} characters.",
                    details={"field": "artifact_names", "index": index, "actual_chars": len(name)},
                )
    ...
    def _validate_review_prompt_fields(template: str | None) -> None:
        ...
        if found != set(REQUIRED_REVIEW_PROMPT_FIELDS):
            raise ConfigurationError(
                "reviewer_prompt placeholders must be exactly {task}, {candidate}, and {artifacts}.",
                details={"field": "reviewer_prompt", "found": tuple(sorted(found))},
            )
        if any(format_spec or conversion for _, name, format_spec, conversion in parsed if name is not None):
            raise ConfigurationError(
                "reviewer_prompt placeholders cannot use conversion flags or format specifications.",
                details={"field": "reviewer_prompt"},
            )
