...
ROLE IN CODEBASE: `vidbyte/providers/typesafe.py` serializes JevDecisionRequest and builds JevAnswer values, `vidbyte/lib/runners/decision.py` passes them through, and `vidbyte/tools/classifier/jev_decide.py` builds requests from model arguments and emits JevDecisionRecord entries.
ARCHITECTURE NOTE: This module must not import model_configs (that would close an import cycle through ModalityDetector); JevDecideSettings lives in jev_settings.py. Records own every shape rule in __post_init__ so a constructed value is provably valid; the provider, not these records, builds the wire dict (lint S060 bars dict[str, Any] encoders here).
...
class JevDecisionRecord:
    """Decision-log entry for one jev_decide call; carries a state hash, never the raw state."""
