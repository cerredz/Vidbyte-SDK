...
PURPOSE: Declares the TypeSafe Jev limits, defaults, and wire literals shared by the decision records, the provider adapter, and the jev_decide tool.
ROLE IN CODEBASE: `vidbyte/lib/dataclasses/jev.py` validates against these bounds, `vidbyte/providers/typesafe.py` builds requests from them, and `vidbyte/tools/classifier/jev_decide.py` reads the same values so the tool and the records can never disagree.
