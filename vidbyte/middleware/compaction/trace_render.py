...
Description:
    Pure renderer that turns a continual-trace artifact dict into bounded Markdown.
Purpose:
    Lets trace-backed compaction inject a readable, size-bounded trace summary into
    the context window without coupling the rendering logic to any runtime state.
Architecture:
    - TraceArtifactRenderer: deterministic artifact -> Markdown renderer with field
      subset, char limit, array head/tail elision, and token-budget trimming.
Relations:
    Used by vidbyte.middleware.compaction.context_compaction.TraceReplacementCompactionMiddleware;
    bounds produced by ReplaceWithTraceCompaction's trace_text option.
"""
