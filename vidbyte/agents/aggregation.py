...
PURPOSE:
    Implements the Multi-Provider Aggregator (Mixture-of-Agents) engine and the AggregateAgent class that exposes it as a first-class SDK agent. Fans a request out to several proposer models concurrently and routes their candidate answers to a final aggregator model that synthesizes a new response grounded in all of them (it composes its own answer, it does not select one).
...
FILE DEPENDENCIES:
    ...
    - vidbyte.lib.dataclasses.multi_agent: imported by this file.
    - vidbyte.lib.enums.prompts: imported by this file.
    - vidbyte.lib.errors: imported by this file.
    - vidbyte.lib.tracing: imported by this file.
    - vidbyte.middleware: imported by this file.

FUNCTION INVENTORY:
    - AggregateResult (class): public or navigational symbol owned here.
    - MultiProviderAggregator (class): public or navigational symbol owned here.
    - AggregateAgent (class): public or navigational symbol owned here.
...
COMMON ERRORS RAISED BY THIS FILE:
    - AggregateExecutionError: raised, returned, or imported by this file. Keep context safe and grepable.
