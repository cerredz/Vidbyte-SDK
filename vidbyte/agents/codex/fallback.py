...
FUNCTION INVENTORY: build(settings) resolves the chain; next_index(decision) asks whether
    ...
    one credential-free try; primary_model(codex) derives chain index 0.
...
class CodexFallbackCoordinator:
    ...
    def model_name(self, index: int) -> str:
        """Return the model name at one chain index, for the answering-model report."""
        return self._model_at(index).model
