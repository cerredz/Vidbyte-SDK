...
PURPOSE:
    Defines the baseline agent implementation (BaseAgent) and configured runner wrappers. Combines prompting, tool registration, runtime state tracking, and execution into a unified developer-facing executable actor (the agent).
...
FILE DEPENDENCIES:
    - vidbyte.agents.mixins: imported by this file.
...
FUNCTION INVENTORY:
    - ConfiguredAgentRunner (class): public or navigational symbol owned here.
    - BaseAgent (class): public or navigational symbol owned here.
...
COMMON ERRORS RAISED BY THIS FILE:
    ...
    - RuntimeError: raised, returned, or imported by this file. Keep context safe and grepable.
    ...
    def __init__(
        ...
        *,
        ...
        if self.runtime_type in (
            ...
            # These runtimes own their control loop, so linear-loop adapters must be rejected at construction.
            if middleware:
        ...
        if self.agent_loop_settings.tool_error_policy is not None and self.runtime_type in (
            ...
            # Tool error policy is implemented as middleware and therefore follows the same linear-loop boundary.
            raise ConfigurationError(
                f"Agent {name} uses non-linear runtime {self.runtime_type.value}, "
