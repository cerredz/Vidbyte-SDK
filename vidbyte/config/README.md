...
Parsing and building are separate steps, and the split is a security boundary rather than a style choice. **Nothing here imports a module, attribute, or callable named by a document** — a document is untrusted text, so nothing in it may name code to execute. `build_agent()` takes exactly what `load_agent()` returned and nothing else:

```python
loader = YamlLoader()
settings = loader.load_agent("agent.yaml")           # parse + validate, no runtime objects
agent = loader.build_agent(settings)                 # construct, resolving refs through the registry
```

A `ref` is not an import path: it is a key looked up in `ComponentRegistry` (`vidbyte.lib.registries`), whose catalogs are the SDK's own public exports from `vidbyte.tools.builtins`, `vidbyte.middleware`, and `vidbyte.context.primitives`. An entry's `options` become that class's keyword arguments. `ComponentRegistry.names("tools" | "middleware" | "context_items")` lists the vocabulary a document may declare. Four rules follow:

- **A document can only name what the SDK registers.** An application's own tools and middleware are not declarable — a YAML file cannot carry a Python callable, so custom components stay the caller's job to attach to the built agent. Naming one raises rather than silently producing an agent without it.
- **Unresolved references fail closed.** An unregistered `ref`, or `options` the registered class rejects, raises `ConfigurationError` naming the entry's document position and the available reference names.
...
- **`name=` is the one override.** It replaces the settings' name and is re-validated against the same rules the document faced; note that renaming settings that came from a harness document decouples the agent's name from that document's `spec_id`. Everything else comes from the settings, so two builds of the same settings differ in nothing but object identity.
...
- It does not replace agent-local tool catalogs in `vidbyte.tools`; declared tools are built into one and handed to the agent.
- It does not let a document name application-defined components; only the classes `ComponentRegistry` catalogs are declarable.
...
- 2026-07-25: Added `YamlLoader.build_agent(settings, *, name=None)`, which consumes what `load()`/`load_agent()` returned — and nothing else — and constructs one `BaseAgent`. Declared `tools`/`middleware`/`context_items` refs resolve through the new `vidbyte.lib.registries.ComponentRegistry` and fail closed on an unregistered ref or rejected options; an output-contract floor naming an absent tool is rejected at build time; `name=` is re-validated through `dataclasses.replace`. Nothing named by a document is ever imported. See `docs/design/yaml-loader-build-agent.md`.
