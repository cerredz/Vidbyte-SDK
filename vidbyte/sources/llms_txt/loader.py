...
Description:
    Defines LlmsTxtSource, the first concrete artifact source.
Purpose:
    Parses an llms.txt index into a compact, always-emitted index primitive and, on request,
    expands selected section links into per-link DocumentContextItems via a shared DocumentSource
    (index-first progressive disclosure).
Architecture:
    - LlmsTxtSource: Source[LlmsTxtDocument] implementing _parse and _emit plus expansion helpers.
Relations:
    Extends vidbyte.sources.base.Source; delegates expansion to vidbyte.sources.document.DocumentSource.
"""
...
from vidbyte.sources._markdown import slugify
...
class LlmsTxtSource(Source[LlmsTxtDocument]):
    ...
    def _build_index_item(self, ir: LlmsTxtDocument, snapshot: SourceSnapshot) -> DocumentContextItem:
        # Renders title, summary, and a per-section link inventory (no bodies) into one labeled item.
        lines = [f"# {ir.title}"]
        if ir.summary:
        ...
        body = "\n".join(lines)
        content = wrap_untrusted_content(body, snapshot.url) if self._label_untrusted else body
    ...
    def _selected_links(self, ir: LlmsTxtDocument, selection: Selection) -> tuple[tuple[int, int, LlmsTxtLink, LlmsTxtSection], ...]:
        # Returns (section_index, link_index, link, section) tuples passing the selection, in document order.
        selected: list[tuple[int, int, LlmsTxtLink, LlmsTxtSection]] = []
        for s_idx, section in enumerate(ir.sections):
            # Optional sections are skipped under a trivial selection (progressive-disclosure default).
