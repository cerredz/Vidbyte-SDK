...
class StagedMypyContractsRule(Rule):
    ...
    severity = "blocking"
    summary = (
        "This rule makes the package-wide mypy result a staged contract rather than an all-or-nothing migration. "
        "Every existing error is counted as explicit debt, while any increase is treated as a new defect in the SDK interface or implementation. "
        "The scan covers production code so provider, transport, runner, tool, and result mismatches cannot hide behind untyped callers. "
        "The ratchet lets the repository improve incrementally without allowing new ambiguity to accumulate."
    )
    impact = (
        "A mypy finding often represents a runtime mismatch even when the failing branch has not been exercised by a test. "
        "Examples include awaiting a synchronous transport, passing an optional value to a required boundary, or returning a result with the wrong public shape. "
        "Allowing new errors makes the type surface less trustworthy for SDK users, IDEs, and future agents. "
        "Raising the baseline would hide that regression and make the remaining debt harder to reduce honestly."
    )
    repair = (
        "Read the full mypy message and inspect both the producer and consumer named by the type mismatch. "
        "Align the implementation with the real contract by correcting annotations, narrowing optional values, awaiting async results, or changing the call shape. "
        "Keep third-party import tolerance limited to missing external stubs and do not add a local ignore or weaken a value to Any. "
        "Run the focused rule, then mypy and the affected source tests, and lower the baseline only after the improvement is intentional."
    )
    examples = (
