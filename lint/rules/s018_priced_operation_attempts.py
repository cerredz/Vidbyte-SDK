...
class PricedOperationAttemptsRule(Rule):
    ...
    severity = "blocking"
    summary = "This rule keeps retry work visible in every priced operation result. It requires successful and failed results to carry the actual request attempts, while validation failures explicitly carry zero because no request occurred. A finding marks a canonical result helper call that can erase whether a provider succeeded immediately, retried, or exhausted its policy. The contract makes usage, pricing, and diagnostic records agree about the work the operation performed. Attempt propagation is required on every executed-result branch, not only on the common success path."
    impact = "Dropping the attempt count underreports provider work and can produce incorrect operation pricing or usage analytics. It also hides retry behavior from callers that need to distinguish a first-attempt result from a degraded or exhausted request. A hard-coded count can look correct in a happy path while becoming false as soon as transport policy changes. The missing metadata therefore weakens both financial correctness and the evidence needed to debug provider reliability. If success and failure branches report different notions of attempts, the same operation can become financially and operationally inconsistent depending on its outcome."
    repair = "Trace each result path back to the request or validation boundary that produced it. Pass response.attempts or payload.attempts for executed requests, client.max_attempts when a retryable request exhausts its policy, and zero for validation failures before I/O. Keep all success and failure construction on the canonical priced helpers so result metadata cannot bypass the accounting contract. Run the focused rule and retry, pricing, and failure-path checks with first-attempt, retried, exhausted, and validation scenarios. Review every early return and exception path so no request can complete without carrying the attempts value that the transport actually observed."
    examples = (
