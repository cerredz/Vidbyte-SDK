...
class ClassBoundRegistryHelpersRule(Rule):
    ...
    severity = "blocking"
    summary = "This rule keeps public behavior for a registry beside the registry state that gives that behavior meaning. It scans declared registry modules for public module-level helpers while allowing private parsing leaves and dunder exports. A finding identifies a helper whose ownership is split from the class that callers must discover to understand the registry contract. The rule makes lookup, validation, normalization, and registry data one navigable public surface. Class ownership gives future changes a visible place to preserve registry invariants and related state."
    impact = "A public free function beside registry data creates two competing discovery surfaces for one conceptual responsibility. Callers may miss the canonical class, duplicate validation logic, or pass state through an API that does not visibly own it. That split becomes more damaging as provider and model catalogues gain aliases, defaults, or compatibility rules. The resulting architecture is harder for agents to navigate and easier to change inconsistently. Ownership drift can also make a helper appear reusable when it actually depends on undocumented registry state or invariants."
    repair = "Identify the registry class whose state and invariants the helper reads or enforces. Move the public helper onto that class as a static method or class method, keeping unrelated parsing details private to the module. Update every caller to use the owning class and preserve the helper's input, output, error, and ordering behavior. Run the focused rule plus registry import, configuration, and caller checks after confirming the public export points at the class-owned surface. Remove obsolete public aliases only after checking downstream imports so the ownership repair does not create an accidental compatibility break."
    examples = (
