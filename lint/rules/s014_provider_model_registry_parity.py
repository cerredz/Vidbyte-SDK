...
class ProviderModelRegistryParityRule(Rule):
    ...
    severity = "blocking"
    summary = (
        "This rule treats provider and model configuration as one declarative catalogue with several parallel views. "
        "It compares provider enum members, default models, API-key names, endpoint maps, qualified runner keys, and bare model runners. "
        "A finding names the exact map or member whose parity has drifted instead of waiting for a late runtime lookup failure. "
        "The rule preserves the intentional bare auto alias while requiring every other provider and model relationship to be explicit."
    )
    impact = (
        "Registry drift can allow configuration validation to accept a provider that dispatch cannot execute. "
        "A default model may lack an endpoint, credential mapping, or qualified runner while appearing valid until the first user request. "
        "That delayed failure is harder to diagnose because the original configuration edit and the runtime symptom may occur in different files. "
        "Incomplete parity can also route a request to the wrong provider, causing incorrect behavior or confusing authentication errors."
    )
    repair = (
        "Read the finding's map name, provider member, model, and reason before editing any registry literal. "
        "Update the enum, defaults, credential environment map, endpoint map, and qualified and bare runner maps together when the provider contract changes. "
        "Add aliases deliberately, preserve the documented bare auto exception, and avoid fallback branches that conceal missing declarative data. "
        "Run the focused rule and the provider configuration and runner smoke checks after confirming every parallel key has the intended value."
    )
    examples = (
