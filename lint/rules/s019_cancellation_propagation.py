...
class CancellationPropagationRule(Rule):
    ...
    severity = "blocking"
    summary = (
        "This rule preserves task cancellation as a control signal through asynchronous SDK boundaries. "
        "It inspects handlers that can catch CancelledError or broad BaseException paths inside provider, tool, runner, and transport work. "
        "A finding identifies a handler that consumes cancellation without re-raising it after any required cleanup. "
        "The contract ensures a caller can stop an abandoned run without leaving hidden work behind."
    )
    impact = (
        "Swallowed cancellation allows provider requests, streams, sockets, tasks, and usage accounting to continue after the caller has abandoned the run. "
        "Those operations can consume capacity, emit late side effects, or hold resources until an unrelated timeout eventually fires. "
        "The resulting behavior makes shutdown nondeterministic and can make a user believe work stopped when it did not. "
        "Because cancellation is represented by control flow rather than a normal result, converting it into fallback success is especially dangerous."
    )
    repair = (
        "Identify whether the handler is responsible for cleanup or is only providing ordinary exception fallback. "
        "Add an explicit CancelledError path that performs required cleanup and re-raises, or re-raise immediately when no cleanup is owned there. "
        "Keep BaseException handling narrow and do not convert cancellation into a success, retry, or ordinary provider error result. "
        "Run the focused rule and the owning task, stream, timeout, and shutdown checks with cancellation injected during active work."
    )
    examples = (
