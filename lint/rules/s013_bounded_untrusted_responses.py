...
class BoundedUntrustedResponsesRule(Rule):
    ...
    severity = "blocking"
    summary = (
        "This rule requires every untrusted ingestion boundary to declare a maximum response size. "
        "It covers search, fetch, browser, code-search, and MCP content before that content becomes an in-memory SDK result or agent context. "
        "A finding identifies a remote body that can be read without a transport-enforced byte ceiling or bounded streaming path. "
        "The rule keeps resource protection at the boundary where the remote party still controls the amount of data received."
    )
    impact = (
        "A remote endpoint controls response size and can send much more data than the operation's normal result requires. "
        "Reading that body in full can exhaust process memory, delay an agent run, or inject an unbounded amount of untrusted text into later reasoning. "
        "Post-read truncation does not prevent the allocation or the network cost, and Content-Length cannot be trusted for every transfer mode. "
        "The missing ceiling therefore creates a denial-of-service and context-integrity risk at a high-leverage boundary."
    )
    repair = (
        "Identify the untrusted response boundary and choose the maximum byte budget from the operation's typed policy or named configuration. "
        "Pass max_response_bytes to the owning HTTP transport and keep enforcement in its bounded streaming or read helper. "
        "For synchronous or MCP transports, add the equivalent bounded-read capability to the adapter before changing the caller. "
        "Run the focused rule and a response-size failure check that proves oversized chunked and ordinary responses stop before full buffering."
    )
    examples = (
