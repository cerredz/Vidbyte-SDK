"""FILE: scripts/test-codex-middleware.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-middleware.md. Runs every test case in that design
...
ROLE IN CODEBASE:
    Loads and runs tests/test_codex_middleware.py's test classes, which in
    turn exercise vidbyte/agents/codex/middleware.py, the middleware
    validation in vidbyte/agents/codex/config.py, and the turn-boundary
    wiring in vidbyte/agents/codex/agent.py. Not imported by any production
    code.
...
ARCHITECTURE NOTE:
    ...
    of sync. Every test runs offline against the real MiddlewarePipeline;
    only the Codex transport is faked.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_middleware.py, add it
    to TEST_CLASSES here in the same change so this script keeps covering
    every test case in the design doc's Testing Plan.

WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_middleware.py, and this script only runs and reports
       on them.
...
RELATED DOCS:
    docs/design/codex-middleware.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-middleware.py`.
"""
...
from tests.test_codex_middleware import (  # noqa: E402
    CodexHarnessAgentMiddlewareTests,
    MiddlewareDecisionTests,
    MiddlewareSettingsValidationTests,
    MiddlewareValidationTests,
)
