"""FILE: scripts/test-codex-failure-recovery.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-failure-recovery.md. Runs every test case in that design
...
ROLE IN CODEBASE:
    Loads and runs tests/test_codex_failure_recovery.py's test classes, which
    in turn exercise vidbyte/agents/codex/failures.py,
...
ARCHITECTURE NOTE:
    ...
    sync. Every test runs offline against the real AgentFallback and Failure
    contracts; only the Codex transport is faked.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_failure_recovery.py,
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_failure_recovery.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/codex-failure-recovery.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-failure-recovery.py`.
"""
...
from tests.test_codex_failure_recovery import (  # noqa: E402
    AgentAttemptLoopTests,
    ClassificationCoverageTests,
    FailureLedgerTests,
    FailureTranslationTests,
    FallbackConstructionTests,
    FallbackDecisionTests,
)
