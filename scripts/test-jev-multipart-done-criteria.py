...
PURPOSE: Runs Jev multipart done-criteria and nested usage-accounting tests with per-case reporting.
...
COMMON MODIFICATION PATTERNS: Add every new design-plan case to its unittest module; this loader runs both modules fully.
...
TESTS: This script runs tests/test_jev_agent.py and tests/test_agent_pricing.py.
"""
...
from tests import test_agent_pricing, test_jev_agent
...
def main() -> int:
    # Loads every Jev and usage test case and returns a shell-friendly status.
    suite = unittest.TestSuite(
        (
            unittest.defaultTestLoader.loadTestsFromModule(test_jev_agent),
            unittest.defaultTestLoader.loadTestsFromModule(test_agent_pricing),
        )
    )
