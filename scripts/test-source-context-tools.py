"""FILE: scripts/test-source-context-tools.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/source-context-tools.md. Runs every test case in that design
...
ARCHITECTURE NOTE:
    ...
    of sync. Every test runs offline against a fake GitHub transport; no test
    touches the real api.github.com.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_source_context_tools.py,
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_source_context_tools.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/source-context-tools.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-source-context-tools.py`.
"""
