"""FILE: scripts/test-codex-speed-tracking.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-speed-tracking.md. Runs every test case in that design
...
ROLE IN CODEBASE:
    Loads and runs tests/test_codex_speed_tracking.py's test classes, which
    in turn exercise vidbyte/agents/codex/speed.py,
    vidbyte/agents/codex/agent.py, vidbyte/agents/codex/result.py, and the
    speed dataclasses in vidbyte/lib/dataclasses/codex.py. Not imported by
    any production code.
...
ARCHITECTURE NOTE:
    ...
    of sync. Every test runs offline against the real AgentSpeedTracker
    driven by an injected clock, so intervals are exact.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_speed_tracking.py,
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_speed_tracking.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/codex-speed-tracking.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-speed-tracking.py`.
"""
...
from tests.test_codex_speed_tracking import (  # noqa: E402
    CodexHarnessAgentSpeedTests,
    CodexSpeedBoundaryValidationTests,
    RecordTurnTests,
)
