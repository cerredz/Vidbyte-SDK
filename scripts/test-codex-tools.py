"""FILE: scripts/test-codex-tools.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-harness-tools.md. Runs every test case in that design
    doc's Testing Plan, printing one PASS/FAIL line per test and a final
    summary line, exiting non-zero on any failure.

ROLE IN CODEBASE:
    Loads and runs tests/test_codex_tools.py's test classes, which exercise
    vidbyte/agents/codex/tools.py and the tool wiring in the Codex agent,
    transport, fork, settings, and ToolsFormatter. Not imported by any
    production code.

ARCHITECTURE NOTE:
    Loads the test module with unittest's TestLoader rather than duplicating
    its assertions, so this script and the pytest suite can never drift out
    of sync. Every test runs offline against a fake Codex SDK; the SDK
    contract tests skip when the optional openai-codex extra is absent.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_tools.py, add it to
    TEST_CLASSES here in the same change.

WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_tools.py.
    2. Do not import this script from production code.

KNOWN EDGE CASES:
    Skipped contract tests are reported as PASS lines because TestResult
    records skips separately from failures and errors.

RELATED DOCS:
    docs/design/codex-harness-tools.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-tools.py`.
"""
...
from tests.test_codex_tools import (  # noqa: E402
    CodexSdkContractTests,
    CodexToolBridgeAttachTests,
    CodexToolCallHandlerClosedLoopTests,
    CodexToolCallHandlerTests,
    CodexToolCallRecordTests,
    CodexToolFormatterTests,
    CodexToolForkTests,
    CodexToolSettingsTests,
    CodexToolTranslatorTests,
