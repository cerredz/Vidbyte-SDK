"""Run every Codex live-control design case with named PASS/FAIL output.
COMMON MODIFICATION PATTERNS: Extend the live control contract with a matching acceptance case.
RELATED DOCS: docs/design/codex-live-control.md
PURPOSE: Execute every live-control acceptance case with named outcomes.
...
class Verification:
    ...
    def run():
        ...
        path = Path(__file__).resolve().parents[1] / "tests/codex_control/test_control.py"
        spec = importlib.util.spec_from_file_location("codex_control_tests", path)
