"""Run every Codex final-acceptance design case with named PASS/FAIL output.
COMMON MODIFICATION PATTERNS: Extend the final acceptance contract with a matching acceptance case.
RELATED DOCS: docs/design/codex-final-acceptance.md
PURPOSE: Execute every final-acceptance acceptance case with named outcomes.
...
class Verification:
    ...
    def run():
        ...
        path = Path(__file__).resolve().parents[1] / "tests/codex_acceptance/test_acceptance.py"
        spec = importlib.util.spec_from_file_location("codex_acceptance_tests", path)
