"""FILE: scripts/test-codex-custom-tools.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-custom-tools.md. Runs every test case in that design
...
ROLE IN CODEBASE:
    Loads and runs tests/test_codex_custom_tools.py's test classes, which in
    turn exercise vidbyte/lib/dataclasses/codex.py tool records, the
    translator and executor in vidbyte/agents/codex/tools.py, and the tools
    wiring in vidbyte/agents/codex/agent.py. Not imported by production code.
...
ARCHITECTURE NOTE:
    ...
    of sync. Every test runs offline with fake tools; no transport or SDK.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_custom_tools.py, add
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_custom_tools.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/codex-custom-tools.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-custom-tools.py`.
"""
...
from tests.test_codex_custom_tools import (  # noqa: E402
    DefinitionValidationTests,
    ExecutorTests,
    FacadeTests,
    TranslatorTests,
)
...
def run_and_report() -> int:
    ...
    suite = unittest.TestSuite(
        loader.loadTestsFromTestCase(cls) for cls in TEST_CLASSES
    )
