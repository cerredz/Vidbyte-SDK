...
def test_templates_contain_no_context_protocols() -> bool:
    # [Edge Case] Asserts that the string contents of the three loaded prompt templates contain NO context protocols.
    try:
        prompts = Prompts()
        intent = prompts.get(Prompt.TEMPLATES_INTENT_BASED)
        persona = prompts.get(Prompt.TEMPLATES_PERSONA)
        ...
        for name, text in [("intent", intent), ("persona", persona), ("specification", specification)]:
            assert "Context Protocol" not in text, f"{name} contains 'Context Protocol'"
            assert "<!--- " not in text, f"{name} contains markdown comment start"
            assert "--->" not in text, f"{name} contains markdown comment end"
        return True
    except AssertionError:
        return False


def test_persona_contains_required_structural_pillars() -> bool:
    # [Hidden Assumption] Asserts that the persona prompt template contains sections matching structural pillars.
    try:
        prompts = Prompts()
        persona = prompts.get(Prompt.TEMPLATES_PERSONA)

        pillars = ["Identity", "Role", "Behavioral Patterns", "Tone", "Knowledge & Expertise", "Intellect"]
        for pillar in pillars:
            assert pillar in persona, f"Persona template is missing pillar: {pillar}"
        return True
    except AssertionError:
        return False


def test_specification_contains_specification_types() -> bool:
    # [Hidden Assumption] Asserts that the specification prompt template contains references to the expanded specification types.
    try:
        prompts = Prompts()
        specification = prompts.get(Prompt.TEMPLATES_SPECIFICATION)
        ...
        spec_types = [
            ...
            "Formatting & Structural Specifications",
            "Constraint & Limit Specifications",
        ]
        for s_type in spec_types:
            assert s_type in specification, f"Specification template is missing type: {s_type}"
        return True
    except AssertionError:
        return False
...
def main() -> None:
    ...
    tests = {
        ...
        "[Edge Case] test_templates_contain_no_context_protocols": test_templates_contain_no_context_protocols,
        "[Hidden Assumption] test_persona_contains_required_structural_pillars": test_persona_contains_required_structural_pillars,
        "[Hidden Assumption] test_specification_contains_specification_types": test_specification_contains_specification_types,
    }
