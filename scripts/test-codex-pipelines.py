"""FILE: scripts/test-codex-pipelines.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-pipelines.md. Runs every test case in that design doc's
    Testing Plan, printing one PASS/FAIL line per test and a final summary
    line, exiting non-zero on any failure, per the design-doc skill's Phase 5
    requirement for an executable script separate from the pytest suite.

ROLE IN CODEBASE:
    Loads and runs tests/test_codex_pipelines.py's test classes, which in turn
    exercise vidbyte/lib/agents/protocol.py, the translation in
    vidbyte/agents/codex/config.py, the entry points and turn lock in
    vidbyte/agents/codex/agent.py, and vidbyte/pipelines/types.py. Not
...
ARCHITECTURE NOTE:
    ...
    sync. Concurrency is proven with an event-gated transport rather than
    sleeps, so the suite stays deterministic.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_pipelines.py, add it to
    TEST_CLASSES here in the same change so this script keeps covering every
    test case in the design doc's Testing Plan.

WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_pipelines.py, and this script only runs and reports
       on them.
...
RELATED DOCS:
    docs/design/codex-pipelines.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-pipelines.py`.
"""
...
from tests.test_codex_pipelines import (  # noqa: E402
    AgentProtocolTests,
    GenerateReplyTests,
    PipelineTopologyTests,
    TranslateInputTests,
    TurnSerializationTests,
)
