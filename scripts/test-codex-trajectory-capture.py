"""Run every Codex trajectory-capture design case with named PASS/FAIL output.
COMMON MODIFICATION PATTERNS: Extend the trajectory capture contract with a matching acceptance case.
RELATED DOCS: docs/design/codex-trajectory-capture.md
PURPOSE: Execute every trajectory-capture acceptance case with named outcomes.
...
class Verification:
    ...
    def run():
        ...
        path = Path(__file__).resolve().parents[1] / "tests/codex_capture/test_capture.py"
        spec = importlib.util.spec_from_file_location("codex_capture_tests", path)
