"""FILE: scripts/test-codex-agent-input-bridge.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-agent-input-bridge.md. Runs every test case in that
    design doc's Testing Plan, printing one PASS/FAIL line per test and a
    final summary line, exiting non-zero on any failure, per the design-doc
    skill's Phase 5 requirement for an executable script separate from the
    pytest suite.

ROLE IN CODEBASE:
    Loads and runs tests/test_codex_agent_input_bridge.py's test classes,
    which in turn exercise vidbyte/agents/codex/config.py,
    vidbyte/agents/codex/agent.py, and the CodexAgentInput alias in
    vidbyte/lib/dataclasses/codex.py. Not imported by any production code.
...
ARCHITECTURE NOTE:
    ...
    of sync. Every test runs offline; the optional openai-codex extra is
    never imported, because the input bridge executes before the transport
    loads the SDK.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_agent_input_bridge.py,
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_agent_input_bridge.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/codex-agent-input-bridge.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-agent-input-bridge.py`.
"""
...
from tests.test_codex_agent_input_bridge import (  # noqa: E402
