"""FILE: scripts/test-codex-output-contracts.py

PURPOSE:
    Standalone Phase-5 verification script for
    docs/design/codex-output-contracts.md. Runs every test case in that design
...
ROLE IN CODEBASE:
    Loads and runs tests/test_codex_output_contracts.py's test classes, which
    in turn exercise vidbyte/agents/codex/contracts.py, the validation call in
    vidbyte/agents/codex/config.py, the evaluation in
    vidbyte/agents/codex/agent.py, and the metadata key in
    vidbyte/agents/codex/result.py. Not imported by any production code.
...
ARCHITECTURE NOTE:
    ...
    sync. Every test runs offline against the real OutputContract floors and
    AgentLoopSettings; only the Codex transport is faked.
...
COMMON MODIFICATION PATTERNS:
    When a new test class is added to tests/test_codex_output_contracts.py,
...
WHAT NOT TO DO IN THIS FILE:
    1. Do not write new assertions here; every assertion belongs in
       tests/test_codex_output_contracts.py, and this script only runs and
       reports on them.
...
RELATED DOCS:
    docs/design/codex-output-contracts.md

TESTS:
    This script is itself a test runner; run it directly with
    `python scripts/test-codex-output-contracts.py`.
"""
...
from tests.test_codex_output_contracts import (  # noqa: E402
    AgentContractIntegrationTests,
    ContractEvaluationTests,
    ContractValidationTests,
    CounterShapeParityTests,
    ToolCounterTests,
    UsageAndOutputCounterTests,
)
