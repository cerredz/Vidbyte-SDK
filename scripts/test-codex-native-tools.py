"""Run every Codex native-tool design case with named PASS/FAIL output.
COMMON MODIFICATION PATTERNS: Extend the native tool contract with a matching acceptance case.
RELATED DOCS: docs/design/codex-native-tools.md
PURPOSE: Execute every native-tool acceptance case with named outcomes.
...
class Verification:
    ...
    def run():
        ...
        path = Path(__file__).resolve().parents[1] / "tests/codex_native_tools/test_native_tools.py"
        spec = importlib.util.spec_from_file_location("codex_native_tools_tests", path)
